...
import { SOURCE_MONDE, REGIONS_DESA, MONDE_2024, RESTE_DANS_SA_REGION,
         CORRIDORS_INTERREGIONAUX, SEUIL_AUTRES_CORRIDORS, COUVERTURE_STATISTIQUE,
         INTRA_AFRICAIN_UA } from './data/mondeData';
...
const PLAIN_TERMS = {
  ...
  desa: {
    ...
    fr: "Le service statistique des Nations unies. Sa base International Migrant Stock, révisée tous les deux ans, couvre 233 pays et territoires ; c’est d’elle que viennent les stocks employés ici.",
    en: 'The United Nations statistical office. Its International Migrant Stock database, revised every two years, covers 233 countries and areas; the stocks used here come from it.',
  },
...
// Marque dans un bloc de texte les notions du glossaire — deux au plus.
// Au-dela, le texte se constelle de mots soulignes et l’oeil ne sait plus ou
// se poser : l’aide devient une gene.
const marquerLesNotions = (texte, lang) => {
...
const LECTURE_REGLAGES = [
  // Le tirage. Trois états et non deux : « Système » ne pose AUCUN attribut, et
  // laisse `prefers-color-scheme` trancher — c'est le défaut, et c'est ce que la
  // plupart des lecteurs auront. Les deux autres posent `data-theme` sur la
  // racine, ce qui l'emporte sur le réglage du système dans les deux sens.
  //
  // Il vit ici, avec l'alignement et l'espacement, parce que c'est la même
  // question : comment vous lisez le mieux. Une bascule séparée dans la barre
  // aurait fait deux endroits pour un seul sujet.
  { cle: 'theme', defaut: 'jour',
    titre: { fr: 'Tirage', en: 'Edition' },
    aide: { fr: "Le jour sur papier, la nuit sur encre.",
            en: 'Day on paper, night on ink.' },
    options: [
      { val: 'jour', lib: { fr: 'Jour', en: 'Day' } },
      { val: 'nuit', lib: { fr: 'Nuit', en: 'Night' } },
    ] },
...
const PrefsLecture = ({ lang }) => {
  ...
  useEffect(() => {
    LECTURE_REGLAGES.forEach(r => {
      // Le tirage ne porte pas le préfixe des autres : la feuille l'attend en
      // `data-theme` sur la racine. Le jour étant l'état par défaut, il n'a rien à
      // poser — seule la nuit s'écrit.
      if (r.cle === 'theme') {
        if (vals.theme === 'nuit') document.documentElement.setAttribute('data-theme', 'nuit');
...
// L'ANCRE DE CITATION.
//
// Le site est bâti pour être cité — la mention (Ben Mokhtar, 2026), le pied de
// citation des exports, les CSV documentés — et pourtant AUCUN titre ne portait
// d'identifiant : on pouvait envoyer le lien d'une section, jamais celui d'une
// analyse précise. Pour un hub scientifique, c'est la fonction qui manque le plus.
//
// Deux décisions de méthode.
//
// L'IDENTIFIANT VIENT DU TITRE, pas d'un compteur. Un « #analyse-3 » se décale dès
// qu'on insère un bloc avant lui, et tous les liens envoyés pointent alors ailleurs.
// Un identifiant tiré du texte survit à la réorganisation ; il ne casse que si l'on
// réécrit le titre, ce qui est justement le moment où il DOIT casser.
//
// LA POSE SE FAIT APRÈS LE RENDU, et se répare toute seule. Ajouter un composant à
// chacun des titres du fichier serait cent points d'appel à tenir à jour ; un
// observateur les couvre tous, y compris ceux qui apparaîtront demain. React
// pouvant reconstruire un titre à tout moment, l'observateur repose ce qu'il
// enlève.
const ardoise = (t) => t
...
const useAncresDeCitation = (lang, tab) => {
  useEffect(() => {
    ...
    const poser = () => {
      ...
      for (const t of main.querySelectorAll('h2, h3')) {
        ...
        // Le signe visible a été retiré à la demande : le titre porte son
        // identifiant, rien de plus. L'ancrage profond continue de fonctionner —
        // c'est l'identifiant qui le porte, pas le bouton.
      }
    };

    poser();
    const obs = new MutationObserver(() => poser());
    obs.observe(main, { childList: true, subtree: true });
    return () => obs.disconnect();
  }, [lang, tab]);

  // Un seul écouteur délégué : les ancres apparaissent et disparaîsent au fil des
  // rendus, leur attacher un gestionnaire chacune reviendrait à les suivre.
  useEffect(() => {
    const surClic = async (e) => {
      const b = e.target.closest?.('.ancre');
      if (!b) return;
      e.preventDefault();
      e.stopPropagation();
      const url = `${location.origin}${location.pathname}#${b.dataset.pour}`;

      // Trois recours, du plus propre au plus ancien. Le presse-papiers moderne
      // demande un contexte sécurisé et une permission ; il n'est pas accordé
      // partout, notamment dans un cadre embarqué. `execCommand` n'en demande
      // aucune. Et si tout échoue, l'ancre passe dans la barre d'adresse, où le
      // lecteur peut la prendre lui-même.
      const marquer = (etat) => {
        b.dataset.copie = etat;
        setTimeout(() => { delete b.dataset.copie; }, 1800);
      };
      ...
      try {
        ...
        if (ok) { marquer('true'); return; }
      } catch { /* on tente le suivant */ }

      location.hash = b.dataset.pour;
      marquer('barre');
    };
  ...
  // ARRIVER PAR UN LIEN PROFOND. Un identifiant ne sert à rien si la cible est
  // repliée : le navigateur saute à un endroit où il n'y a rien à voir. On déplie
  // donc le chapitre qui la contient avant de s'y rendre. Le délai laisse la
  // section finir son rendu — la cible n'existe pas encore à l'instant du montage.
  useEffect(() => {
    const aller = () => {
      ...
      if (!cible) return;
      const chapitre = cible.closest('.chapitre[data-replie="true"]');
      if (chapitre) chapitre.querySelector('.chapitre-pliure')?.click();
      requestAnimationFrame(() => cible.scrollIntoView({
        block: 'start',
        behavior: prefersReducedMotion() ? 'auto' : 'smooth',
      }));
    };
    const t = setTimeout(aller, 420);
    window.addEventListener('hashchange', aller);
    return () => { clearTimeout(t); window.removeEventListener('hashchange', aller); };
  }, [lang, tab]);
};
...
// ---------- LE RANG CONTINENTAL -------------------------------------------
//
// La fiche disait tout du pays et rien de sa place. Or c’est la première
// question d’un lecteur devant un profil : 7,49 % du PIB en transferts, est-ce
// beaucoup ? Le chiffre seul ne répond pas. Le rang répond, et la bande le
// montre — cinquante-quatre traits, un par État, celui du pays marqué, la
// médiane pour repère.
//
// L’axe porte le CLASSEMENT, pas la valeur. Sur des séries aussi dissymétriques
// que le stock migratoire — de soixante mille à plusieurs millions — une échelle
// de valeurs entasse cinquante traits sur le premier dixième et ne se lit plus.
// L’intitulé dit « rang », donc rien n’est travesti.
//
// Les positions sont posées en `insetInlineStart` : en arabe la bande se lit de
// droite à gauche, et le premier rang doit rester du côté où commence la lecture.
const nombreOuNull = (x) => {
  if (x === null || x === undefined) return null;
  const v = typeof x === 'number' ? x : parseFloat(String(x).replace(/[   ]/g, '').replace(',', '.'));
...
const RangContinental = ({ valeur, serie, libelle, lang, teinte = 'bg-blue-700', encre = 'text-blue-800' }) => {
  ...
  if (v === null || valeurs.length < 5) return null;
...
// LE SCEAU : la marque réduite à ce qui reste lisible à seize pixels.
//
// « Utilise des éléments du logo pour esthétiser les boutons et les défilements. »
// La marque entière ne le permet pas : à cette taille, les cinquante-quatre pays
// du continent deviennent une tache et le dégradé de l’anneau une bouillie. On
// n’en garde que deux composantes — l’anneau brèché et le point qui ferme la
// brèche — en une seule encre, celle du texte qui l’accompagne.
//
// Le tracé vient de BrandMark à l’identique : même vue, mêmes coordonnées. Ce
// n’est pas un dessin ressemblant, c’est le même dessin.
const SceauMarque = ({ className = "w-4 h-4" }) => (
  <svg viewBox={MARQUE_VUE} className={className} fill="none" aria-hidden="true" focusable="false">
    <path d="M89.84 73 A46 46 0 1 1 89.84 27" stroke="currentColor" strokeWidth="6" />
    <circle cx="96" cy="50" r="10" fill="currentColor" />
  </svg>
);
...
// ============================================================================
// LE FEUILLETAGE : UN CARROUSEL QU'ON TIRE À LA MAIN
// ----------------------------------------------------------------------------
// Le rail précédent défilait à la molette et par deux flèches. Il marchait,
// mais rien n'y disait quelle carte comptait : les six visibles avaient le même
// poids, et l'œil devait choisir seul.
//
// Ici la position PORTE l'information. La carte au centre est nette, à pleine
// échelle et pleine ombre ; ses voisines reculent — un peu plus petites, un peu
// plus pâles, légèrement tournées sur l'axe vertical. On sait donc en un regard
// laquelle est lue, et laquelle vient ensuite.
//
// Trois choses valent d'être expliquées.
//
// LA MESURE. Toute la mécanique repose sur deux nombres relevés dans le DOM :
// le PAS (largeur d'une carte plus la gouttière) et le CENTRE de la fenêtre.
// Ils changent au redimensionnement, et le carrousel les reprend alors — sans
// quoi le calage se décale dès qu'on tourne un téléphone.
//
// LA PROJECTION. Au relâchement, on ne cale pas sur la carte la plus proche de
// l'endroit où le doigt s'est levé : on cale sur celle où l'élan AURAIT mené.
// C'est ce que fait `info.velocity.x * 0.18` — sans quoi un geste rapide et
// court se voit annulé, ce qui donne l'impression que le carrousel résiste.
//
// LE MOUVEMENT RÉDUIT. `prefers-reduced-motion` n'est pas un détail
  ...
  return (
    ...
    >
      <button
        ...
        style={{ '--teinte-cran': t.color }}
      >
        <span className="feuillet-etiquettes">
          <span className="pastille pastille--cran">
            <RobustnessMeter level={fiche.confidence_level} />
            {tr(fiche.verdict, lang)}
          </span>
          <span className="pastille pastille--theme">
            <CatIcon className="w-3 h-3 shrink-0" aria-hidden="true" />
            {tr(fiche.category, lang)}
          </span>
        </span>

        <span className="feuillet-enonce">{tr(fiche.narrative, lang)}</span>
...
const Carrousel = ({ items, carte: Carte, lang, choisie, onChoisir, etiquette, className = '' }) => {
  ...
  const [mesure, setMesure] = useState({ pas: 1, carte: 1, centre: 0 });
  const [pos, setPos] = useState(0);
  const reduit = prefersReducedMotion();
  const total = items.length;

  // --- La mesure ----------------------------------------------------------
  // Le pas se relève sur la première carte plutôt que d'être posé en dur : la
  // largeur est fixée par la feuille de style, qui la fait varier avec l'écran.
  useLayoutEffect(() => {
    ...
    const relever = () => {
      ...
      const gouttiere = parseFloat(getComputedStyle(fenetre.firstElementChild).columnGap) || 0;
      // Les cartes sont posees a partir de l'origine du CONTENU de la fenetre,
      // donc apres sa marge interieure. Le centre doit se mesurer dans le meme
      // repere, sans quoi il est decale de cette marge et aucune carte ne
      // tombe jamais exactement sur l'axe de la page.
      const marge = parseFloat(style.paddingInlineStart) || 0;
      setMesure({
        pas: largeur + gouttiere,
        carte: largeur,
        centre: fenetre.clientWidth / 2 - marge,
      });
    };
    relever();
    const obs = new ResizeObserver(relever);
    obs.observe(fenetre);
    return () => obs.disconnect();
  }, [total]);
  ...
  // Le carrousel suit la fiche ouverte, d'où qu'elle vienne — un clic dans la
  // liste, un filtre qui change, un retour d'historique.
  useEffect(() => {
    const i = items.findIndex((f) => f.id === choisie);
    if (i >= 0 && i !== pos) aller(i);
    else if (i >= 0) x.set(cible(i));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [choisie, mesure, items]);

  // --- Le relâchement -----------------------------------------------------
  const surFin = (_e, info) => {
    // On cale là où l'élan aurait mené, et non là où le doigt s'est levé.
    const projection = x.get() + info.velocity.x * 0.18;
    const i = Math.round((mesure.centre - mesure.carte / 2 - projection) / mesure.pas);
    const j = aller(i);
        ...
        <div className="px-6 md:px-8 py-6 space-y-6 text-sm text-slate-700 leading-relaxed">
          ...
          <div className="aparte">
            ...
            <p className="text-[11px] mt-3 leading-relaxed" style={{ color: 'var(--label)' }}>
              <span className="surtitre">{L('Au rythme de métronome :', 'Metronomic:')}</span>{' '}
      ...
      <section className="bg-white" style={{ borderStyle: 'solid', borderColor: 'var(--rule)', borderWidth: 1, borderTopWidth: 2, borderTopColor: 'var(--accent)' }}>
        ...
        <div className="px-6 md:px-8 py-6 space-y-6 text-sm text-slate-700 leading-relaxed">
          ...
          <div className="aparte">
            ...
            <Prose className="text-[13px] text-slate-600 leading-relaxed text-justify" lang={lang}>{L(
                "Ces quatre États sont voisins, et leur calendrier s’est resserré sur dix-huit mois : la Centrafrique en décembre 2025, le Cameroun en avril-mai 2026, le Gabon en mai, le Tchad en juin-août. Trois de ces opérations sont les premières entièrement numériques de leur pays ; deux sont couplées à un recensement agricole.\n\nLues à travers le seul cycle 2020, elles comptent pour zéro et alimentent le récit du déficit. Lues pour ce qu’elles sont, elles ouvrent la série 2030 — et sortent l’Afrique centrale d’une interruption qui durait, selon les pays, de treize à vingt-deux ans.\n\nLe découpage décennal sert la comparaison internationale ; il ne mesure pas l’effort statistique national. Il pénalise même les États dont l’opération a été retardée par un conflit ou par un financement tardif (Ben Mokhtar, 2026).",
                "These four states are neighbours, and their calendars converged within eighteen months: the Central African Republic in December 2025, Cameroon in April-May 2026, Gabon in May, Chad in June-August. Three of these operations are their country’s first fully digital census; two are paired with an agricultural census.\n\nRead through the 2020 round alone, they count as zero and feed the deficit narrative. Read for what they are, they open the 2030 series — and pull Central Africa out of a gap that had lasted, depending on the country, from thirteen to twenty-two years.\n\nThe decennial split serves international comparison; it does not measure national statistical effort. It even penalises states whose operation was delayed by conflict or by late funding (Ben Mokhtar, 2026)."
              )}</Prose>
          </div>
        </div>
  
        <div className="px-6 md:px-8 py-4" style={{ backgroundColor: 'var(--paper-sunk)', borderTop: '1px solid var(--rule)' }}>
          <Prose className="note-source" lang={lang}>{L(
...
// LE TEMPS D’UNE DÉMONSTRATION, ET SON RANG DANS LA SÉRIE.
//
// « Il y a parfois des numéros de sous-section et parfois non. » C’est exact, et
// c’était illisible : la numérotation vivait dans Données et dans un seul volet
// sur quatre de Mobilités, sans qu’aucune règle ne se laisse voir.
//
// La règle existe pourtant, et elle est bonne — on ne numérote QUE ce qui se lit
// dans l’ordre. Données tient une démonstration en trois temps (le constat, le
// décrochage, la réponse) ; les Corridors en tiennent une en deux. Les autres
// sections alignent des vues parallèles d’un même objet : les numéroter
// inventerait un ordre qui n’existe pas, et un numéro qui ne veut rien dire est
// l’ornement que l’on chasse partout ailleurs.
//
// Ce qui manquait, c’est que la règle se VOIE. « 01 » seul est un ornement ;
// « 01/03 » annonce une démonstration en trois temps et dit où l’on en est. Le
// total est donc obligatoire — sans lui, le composant retombe dans le défaut
// qu’il vient de corriger.
const MovementOpener = ({ n, sur, kicker, thesis, accent = 'var(--accent-deep)' }) => (
  <div>
    <div className="flex items-baseline gap-4 mb-3">
      <span className="font-serif font-black text-[2.6rem] leading-none tabular-nums" style={{ color: accent }}>
        {n}
        {sur && (
          <span className="text-[1.1rem] font-bold" style={{ color: 'var(--ink-mute)' }}>
            /{sur}
          </span>
        )}
      </span>
...
const CHIFFRES_DU_CORPUS = () => ({
  etats: ASSISE.etats, references: ASSISE.references,
  notions: ASSISE.notions, enonces: ASSISE.enonces,
});
const poserLesChiffres = (t) => (typeof t === "string"
...
const PageHeader = ({ badge, title, highlight, desc, plate, plain, lang = 'fr', icon: Icon = Globe }) => (
  ...
  <header
    ...
    className="page-bandeau relative overflow-hidden rounded-xl px-6 lg:px-12 py-14 md:py-16"
  >
    ...
    {/* Voile d’encre cote texte : la planche emerge vers la droite au lieu de
        ...
        Il était peint avec `var(--ink)`. De jour l'encre est sombre, donc le
        voile assombrissait ; de nuit elle devient claire, et le voile posait un
        dégradé BLANC en travers du titre de section. C'est le plan encré qu'il
        lui faut — une surface, pas une couleur de texte. Le second arrêt, écrit
        en dur rgba(20,17,15,.72), se compose maintenant du même plan. */}
    <div
      className="absolute inset-0 pointer-events-none"
      style={{ background: 'linear-gradient(90deg, var(--plan-encre) 18%, color-mix(in oklab, var(--plan-encre) 72%, transparent) 45%, transparent 78%)' }}
    />
    ...
    <div className="relative z-10 max-w-4xl">
      ...
      >
        {poserLesChiffres(title)}{' '}
        {/* La seconde famille, ici et nulle part ailleurs : la reprise du titre
            se dit dans la moderne quand le constat se dit dans la graphique. */}
        <span className="titre-reprise" style={{ color: 'var(--accent-light)' }}>{poserLesChiffres(highlight)}</span>
      </h1>
  ...
  {/* Le bloc « En clair » a été retiré : il redisait le descriptif dans quatre
      sections, et définissait le mot-clé de la section dans les autres — ce que
      les « Repères », juste en dessous, font déjà et mieux. Une section s'ouvre
      donc sur un titre et un paragraphe, rien de plus. */}
  </>
);
...
const BarreSection = ({ lang, children = null }) => (
  // La barre porte son nom pour que la mise en page puisse la placer. Elle est
  // appelee en tete de section dans treize composants ; plutot que de deplacer
  // treize appels, la feuille de style la renvoie en pied. Le resultat est le
  // meme partout, et il ne peut pas deriver d’une section a l’autre.
  <div className="barre-section flex flex-wrap items-center justify-end gap-2 print:hidden">
    {children}
    <button
      type="button"
      ...
      <div className="pdf-cols">
        ...
        {display.normlex && (
          <div className="pdf-block">
            ...
            <table><tbody>
              <tr><th scope="row" className="k">{L('Fondamentales', 'Fundamental')}</th><td className="v">{display.normlex.fundamental} / 11</td></tr>
              <tr><th scope="row" className="k">{L('Gouvernance', 'Governance')}</th><td className="v">{display.normlex.governance} / 4</td></tr>
              <tr><th scope="row" className="k">{L('Techniques', 'Technical')}</th><td className="v">{display.normlex.technical}</td></tr>
              <tr><th scope="row" className="k">{L('Total ratifiées', 'Total ratified')}</th><td className="v">{display.normlex.total}</td></tr>
            </tbody></table>
          </div>
        )}
...
const t = {
    fr: {
      ...
      // Le champ `plain` de chaque section n’est plus rendu : le bloc
      // « En clair » a été retiré du bandeau (il redisait le descriptif, ou
      // définissait un mot que les « Repères » définissent déjà). Les textes
      // restent ici, traduits, tant qu’on n’a pas décidé de les supprimer.
      headers: {
        climat: {
          badge: "Mobilité climatique",
          title: "Le climat déplace dix fois moins que le conflit,",
          highlight: "et davantage dans dix-neuf États.",
        ...
        corridors: {
          ...
          title: "Les corridors intra-africains portent l’essentiel,",
          highlight: "et restent les moins documentés.",
        ...
        evidence: {
          ...
          title: "{enonces} affirmations courantes,",
          highlight: "passées aux meilleures sources disponibles.",
          desc: "{enonces} affirmations publiques, reprises une par une et confrontées aux sources qui permettent de les vérifier. La note porte sur la solidité de la preuve disponible, et sur elle seule.",
        ...
        data: {
          ...
          title: "L’Afrique produit des données,",
          highlight: "le déficit se situe en aval.",
          desc: "Le récit d’une Afrique sans données est courant et rarement vérifié. Cette section le confronte au volume réel de la production statistique du continent, puis situe le déficit là où il se trouve : dans ce qui remonte des services nationaux vers les agrégats internationaux.",
        ...
        library: {
          ...
          title: "Les sources mobilisées par la plateforme,",
          highlight: "et le lien vers chacune.",
          desc: "{references} références — rapports institutionnels, bases de données, thèses, textes juridiques. Chaque chiffre du site remonte à l’une d’elles, et chaque entrée porte son lien vers la source.",
        ...
        forced: {
          ...
          highlight: "et que les statistiques migratoires excluent.",
          desc: "Déplacés internes, réfugiés, apatrides, déplacement lié aux catastrophes : les formes contraintes de mobilité sont, en Afrique, massivement internes. Elles relèvent d’instruments africains antérieurs et plus larges que les cadres mondiaux, et c’est là que l’écart entre la norme et son ancrage national est le plus large.",
    ...
    en: {
      ...
      headers: {
        climat: {
          ...
          title: "Climate displaces ten times less than conflict,",
          highlight: "and more than it in nineteen states.",
        ...
        corridors: {
          ...
          title: "Intra-African corridors carry most of the movement,",
          highlight: "and remain the least documented.",
        ...
        evidence: {
          ...
          title: "{enonces} common claims,",
          highlight: "tested against the best available sources.",
          desc: "{enonces} public claims, taken one at a time and set against the sources that make them checkable. The score rates the strength of the available evidence, and nothing else.",
        ...
        data: {
          ...
          title: "Africa does produce data,",
          highlight: "the deficit sits downstream.",
        ...
        governance: {
          ...
          title: "Africa built its own instruments, often ahead of the global norm.",
          highlight: "What states do with them is another matter.",
          desc: "Six continental instruments, {etats} states, one ratification table. The Constitutive Act binds every one of them; the Free Movement Protocol binds four.",
        ...
        library: {
          ...
          title: "The sources this platform draws on,",
          highlight: "and the link to each one.",
          desc: "{references} references — institutional reports, databases, theses, legal instruments. Every figure on the site traces back to one of them, and every entry carries its link to the source.",
        ...
        forced: {
          ...
          highlight: "and that migration statistics leave out.",
          desc: "Internally displaced people, refugees, stateless persons, disaster displacement: in Africa the constrained forms of mobility are overwhelmingly internal. They fall under African instruments that predate and exceed the global frameworks, and it is there that the gap between norm and national anchoring is widest.",
...
const aggregates = {
  'africa_perspective': {
    name: { fr: "Afrique (Continentale)", en: "Africa (Continental)" }, flagIcon: Globe, flagColor: "text-blue-700", stock: '28,5 M', female: '47.1', evolution: '1.9', retention: INTRA_AFRICAIN_UA.part, remittances: 6.5, aid: 3.2,
    history: [{ year: 1990, value: '2.5' }, { year: 2000, value: '2.0' }, { year: 2010, value: '1.8' }, { year: 2024, value: '1.9' }],
    distribution: [{ label: {fr: 'Intra-Africain', en: 'Intra-African'}, value: INTRA_AFRICAIN_UA.part, color: 'bg-blue-700' }, { label: {fr: 'Extra-Africain', en: 'Extra-African'}, value: INTRA_AFRICAIN_UA.partHorsContinent, color: 'bg-slate-700' }],
    ...genericDesc
  },
  'af_med_perspective': {
...
const mapIndicators = [
  ...
  },
  {
    ...
    hint: { fr: "Part des mobilités qui restent dans un pays du Sud plutôt que de rejoindre le Nord (UA, OIT, OIM et CEA, 2021).", en: "Share of mobility remaining within the Global South rather than moving North (AU, ILO, IOM and ECA, 2021)." },
    plain: { fr: "Plus la teinte est dense, plus la part des partants restés sur le continent est élevée. Un pays clair envoie l’essentiel des siens au-delà de l’Afrique.", en: "The denser the shade, the higher the share of leavers who stayed on the continent. A pale country sends most of its own beyond Africa." },
    term: 'retention'
  },
  {
    ...
    plain: { fr: "Plus la teinte est dense, plus le pays ouvre sa frontière aux autres Africains. Le blanc marque une frontière fermée à presque tous.", en: "The denser the shade, the more the country opens its border to other Africans. White marks a border closed to nearly everyone." },
    term: 'avoi'
  },
  {
    ...
    hint: { fr: "Transferts des diasporas rapportés au PIB (Banque mondiale, 2024).", en: "Diaspora remittances as a share of GDP (World Bank, 2024)." },
    plain: { fr: "L’argent envoyé au pays par ceux qui vivent à l’étranger, rapporté à la richesse produite sur place. Sur les 48 États africains pour lesquels la Banque mondiale publie la mesure, ces transferts dépassent 5 % du produit intérieur brut dans seize d’entre eux, soit davantage que l’aide publique au développement reçue.", en: "Money sent home by people living abroad, measured against the wealth produced locally. Across the 48 African states for which the World Bank publishes the measurement, these transfers exceed 5 per cent of gross domestic product in sixteen of them, more than the official development assistance received." }
  },
  {
    key: 'idp_conflict', label: { fr: "Déplacés internes (conflit)", en: "IDPs (conflict)" },
    unit: '', get: c => Number(c.idp_conflict),
    hint: { fr: "Personnes déplacées à l’intérieur du pays par les conflits (IDMC, GRID 2025).", en: "People displaced within the country by conflict (IDMC, GRID 2025)." },
  ...
  // charge de la protection sur le continent.
  {
    ...
    hint: { fr: "Personnes déplacées à l’intérieur du pays par une catastrophe (IDMC, GRID 2025).", en: "People displaced within the country by a disaster (IDMC, GRID 2025)." },
    plain: { fr: "Le nombre de personnes chassées de chez elles par une inondation, une sécheresse ou une tempête, et restées dans leur pays. Contrairement à ce qu’on lit souvent, cette mobilité ne relève pas d’une projection : elle est enregistrée chaque année, pays par pays, depuis 2008.", en: "The number of people driven from home by flood, drought or storm and still inside their country. Contrary to what is often written, this mobility is not a projection: it has been recorded annually, country by country, since 2008." }
  },
  {
...
const AfricaChoropleth = ({ indicator, lang, selectedId, onSelect, compact = false, region = null }) => {
  ...
  const fmt = (v) => {
    ...
    // La branche avec unité sautait le formateur : « 13.2% » au lieu de
    // « 13,2 % ». Un séparateur décimal anglais sur une carte française se voit.
    if (indicator.unit === '') return formatNumber(v, lang);
    return `${formatNumber(v, lang)} ${indicator.unit}`;
  };
  ...
  // LE BROSSAGE DE LA LÉGENDE.
  //
  // Une légende de choroplèthe dit à quoi correspond chaque teinte, et s'arrête
  // là. Elle laisse donc sans réponse la question qu'on se pose vraiment devant
  // une carte : QUI est dans cette bande ? On la lit alors teinte par teinte, en
  // balayant le continent des yeux, et l'on se trompe.
  //
  // Chaque cran devient un bouton. Le survol éteint tout ce qui n'est pas de la
  // classe ; le clic l'épingle, pour qu'on puisse lâcher la souris et lire. Le
  // cran porte aussi le NOMBRE de pays qu'il contient — une information que la
  // carte ne donne pas d'elle-même, et qu'il faudrait sinon compter à la main.
  const [classeSurvolee, setClasseSurvolee] = useState(null);
  const [classeEpinglee, setClasseEpinglee] = useState(null);
  const classeActive = classeSurvolee ?? classeEpinglee;

  // LA SONDE. Le relevé latéral est riche mais loin : l'œil fait l'aller-retour
  // entre le pays pointé et le panneau. Une étiquette au curseur garde la main et
  // le regard au même endroit ; le panneau reste pour le détail.
  const [sonde, setSonde] = useState(null);
  const cadre = useRef(null);
  const suivre = (e, id) => {
    const b = cadre.current?.getBoundingClientRect();
    if (!b) return;
    setSonde({ x: e.clientX - b.left, y: e.clientY - b.top, id });
  };
  ...
  // LA COMPARAISON À DEUX.
  //
  // Un atlas de cinquante-quatre États n'ouvre qu'une fiche à la fois, et le geste
  // analytique premier — mettre deux pays côte à côte — demandait deux onglets et
  // une mémoire. Elle vit dans le relevé plutôt que dans une surface de plus : le
  // panneau sait déjà lire un pays, il en lit deux.
  //
  // On s'arrête à DEUX. Trois colonnes dans une bande de quinze rem donnent des
  // chiffres qu'on ne compare plus, on les déchiffre ; et l'écart, qui est ce
  // qu'on vient chercher, n'a de sens qu'entre deux termes.
  const [duo, setDuo] = useState([]);
  const enDuo = duo.length === 2;
  const ajouterAuDuo = (id) => setDuo(d =>
    d.includes(id) ? d.filter(x => x !== id) : (d.length >= 2 ? [d[1], id] : [...d, id]));

  // À quelle classe appartient un pays, et combien sont-ils par classe.
  // On calcule sur les pays DU CADRE : sur une carte de sous-région, compter le
  // continent entier donnerait un nombre qui ne correspond à rien de visible.
  const classeDe = (id) => {
    const v = valueOf(id);
    if (categoriel) return Number.isFinite(v) || typeof v === 'string' ? String(v) : null;
  ...
  const comptes = useMemo(() => {
    const t = {};
    for (const id of Object.keys(africaCountryPaths)) {
      if (dansLeCadre && !dansLeCadre.has(String(id))) continue;
      const c = classeDe(id);
      if (c === null) continue;
      t[c] = (t[c] || 0) + 1;
    }
    return t;
  }, [valueOf, categoriel, buckets, dansLeCadre]);

  // Un pays s'éteint quand une classe est brassée et qu'il n'en est pas.
  const eteint = (id) => classeActive !== null && classeDe(id) !== classeActive;

  const brosser = (cle) => ({
    onMouseEnter: () => setClasseSurvolee(cle),
    onMouseLeave: () => setClasseSurvolee(null),
    onFocus: () => setClasseSurvolee(cle),
    onBlur: () => setClasseSurvolee(null),
    onClick: () => setClasseEpinglee(p => (p === cle ? null : cle)),
    'aria-pressed': classeEpinglee === cle,
    className: 'choro-cran',
    type: 'button',
  });

  // Les mêmes quatre lignes pour un pays, quel qu'il soit : le relevé simple les
  // affiche pour celui qu'on pointe, la comparaison pour chacun des deux.
  const lignesDe = (id) => {
    const c = countryById[id];
    if (!c) return [];
    return [
      { l: tr(indicator.label, lang), v: fmt(valueOf(id)), brut: valueOf(id), fort: true,
        rang: rangDe(id), surTotal: rangs?.total },
      { l: L('Migrants internationaux', 'International migrants'), v: formatNumber(c.stock, lang),
        brut: parseFloat(String(c.stock).replace(/[^0-9.]/g, '')) },
      { l: L('Ouverture des visas', 'Visa openness'), v: c.avoi == null ? '—' : `${c.avoi}/100`, brut: c.avoi },
      { l: L('Rétention Sud-Sud', 'South-South retention'), v: c.retention == null ? '—' : `${c.retention} %`, brut: c.retention },
    ];
  };

  const releve = paysLu ? lignesDe(lu) : [];

  return (
    <div className="carte-continent grid lg:grid-cols-[1fr_15rem] gap-4 items-start"
         ref={cadre}
         data-brossage={classeActive !== null ? 'true' : 'false'}>
      <svg viewBox={(region && CADRES_REGIONS[region]) ? CADRES_REGIONS[region].join(' ') : AFRICA_VIEWBOX}
           ...
           onMouseLeave={() => { setSurvole(null); setSonde(null); }}>
        {Object.entries(africaCountryPaths).map(([id, d]) => {
          ...
          return (
            <path
              ...
              className={hors ? 'choro-pays choro-hors'
                              : `cursor-pointer choro-pays${eteint(id) ? ' choro-eteint' : ''}`}
              tabIndex={hors ? -1 : 0}
              role={hors ? 'presentation' : 'button'}
              aria-hidden={hors ? 'true' : undefined}
              aria-label={hors ? undefined : `${nom} — ${fmt(v)}`}
              onMouseMove={hors ? undefined : (e) => suivre(e, id)}
        ...
        {PETITS_ETATS.map(([id, cx, cy]) => {
          ...
          return (
            <circle
              ...
              className={`cursor-pointer choro-pays choro-amorce${eteint(id) ? ' choro-eteint' : ''}`}
              tabIndex={0}
              role="button"
      ...
      {/* LA SONDE. Le relevé latéral est riche, mais il est loin : l'œil fait
          l'aller-retour entre le pays pointé et le panneau, et l'on perd le pays
          de vue en lisant son chiffre. Une étiquette au curseur garde la main et
          le regard au même endroit. Elle ne dit que le strict minimum — le nom et
          la valeur ; le détail reste au panneau, qui ne bouge pas. */}
      {sonde && countryById[sonde.id] && (
        <span className="carte-sonde" aria-hidden="true"
              style={{ insetInlineStart: `${sonde.x}px`, top: `${sonde.y}px` }}>
          <b>{tr(countryById[sonde.id].name, lang)}</b>
          <i>{fmt(valueOf(sonde.id))}</i>
        </span>
      )}
      ...
      <aside className="border p-4" style={{ borderColor: 'var(--rule)', backgroundColor: 'var(--paper-raised)' }}
             aria-live="polite">
        {/* LA COMPARAISON. Deux colonnes, les mêmes quatre lignes, et l'écart —
            qui est ce qu'on vient chercher. Elle prend la place du relevé simple
            plutôt que de s'ajouter à côté : deux panneaux qui disent la même chose
            à deux endroits, c'est ce qu'on passe ses journées à supprimer. */}
        {enDuo ? (
          <>
            <div className="flex items-baseline justify-between gap-2 mb-3">
              <span className="surtitre">
                {L('Comparaison', 'Comparison')}
              </span>
              <button type="button" className="duo-fermer" onClick={() => setDuo([])}>
                {L('fermer', 'close')}
              </button>
            </div>
            <div className="duo-tete">
              {duo.map(id => (
                <span key={id} className="duo-nom">{tr(countryById[id]?.name, lang) || id}</span>
              ))}
            </div>
            <dl className="duo-corps">
              {lignesDe(duo[0]).map((ra, k) => {
                const rb = lignesDe(duo[1])[k];
                const a = ra.brut, b = rb ? rb.brut : null;
                const chiffres = Number.isFinite(a) && Number.isFinite(b);
                return (
                  <div key={k} className="duo-ligne">
                    <dt>{ra.l}</dt>
                    <dd>
                      <b className={chiffres && a > b ? 'duo-fort' : undefined}>{ra.v}</b>
                      <b className={chiffres && b > a ? 'duo-fort' : undefined}>{rb ? rb.v : ''}</b>
                    </dd>
                    {chiffres && a !== b && (
                      <span className="duo-ecart">
                        {L('écart', 'gap')} {formatNumber(Math.round(Math.abs(a - b) * 10) / 10, lang)}
                      </span>
                    )}
                  </div>
                );
          ...
          <>
            ...
            <button type="button" className="duo-appel" onClick={() => ajouterAuDuo(lu)}>
              {duo.includes(lu)
                ? L('Retirer de la comparaison', 'Remove from comparison')
                : duo.length === 1
                  ? L('Comparer avec ce pays', 'Compare with this country')
                  : L('Comparer ce pays', 'Compare this country')}
              {duo.length === 1 && !duo.includes(lu) && (
                <em className="duo-attente">{tr(countryById[duo[0]]?.name, lang)}</em>
              )}
            </button>
          </>
        ) : (
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-2 pt-2 border-t border-slate-100 lg:col-span-2">
        <span className="surtitre" style={{ color: 'var(--label)' }}>
          {tr(indicator.label, lang)}
        </span>
        {categoriel ? (
          indicator.categories.map(c => (
            <button key={c.key} {...brosser(String(c.key))}>
              ...
              {comptes[String(c.key)] > 0 && <em className="choro-compte">{comptes[String(c.key)]}</em>}
            </button>
          ))
        ) : (
          <>
            <span className="flex items-center gap-0.5">
              {CHORO_RAMP.map((c, i) => (
                <button key={i} {...brosser(String(i))}
                        aria-label={L(`Classe ${i + 1} sur ${CHORO_RAMP.length} — ${comptes[String(i)] || 0} pays`,
                                      `Class ${i + 1} of ${CHORO_RAMP.length} — ${comptes[String(i)] || 0} countries`)}>
                  <span className="w-6 h-2.5 rounded-[1px]" style={{ background: c }} />
                  <em className="choro-compte">{comptes[String(i)] || 0}</em>
                </button>
              ))}
            </span>
        ...
        {/* Ce que le brossage vient de répondre, dit en toutes lettres : sans cela,
            l'utilisateur voit la carte s'éteindre sans savoir ce qu'il a demandé. */}
        {classeActive !== null && (
          <span className="choro-brosse-dit">
            {L(`${comptes[classeActive] || 0} pays dans cette classe`,
               `${comptes[classeActive] || 0} countries in this class`)}
            {classeEpinglee !== null && (
              <button type="button" className="choro-liberer"
                      onClick={() => setClasseEpinglee(null)}>
                {L('tout rallumer', 'show all')}
              </button>
            )}
          </span>
        )}
...
// L’apostrophe est repliée avec les accents : la prose porte la courbe (’),
// que personne ne tape au clavier. Sans ce repli, chercher « droit d’asile »
// ne trouverait plus « droit d’asile », qui est pourtant ce qui est écrit.
...
//
// « Données & statistiques » faisait quinze écrans d’affilée. Le lecteur y
// descendait en espérant croiser ce qui l’intéresse, sans jamais savoir ce qui
// restait devant lui. Un sommaire aurait ajouté une couche de navigation ; on
// préfère que chaque analyse annonce ce qu’elle contient et se déplie sur
// demande.
//
// LA COUPE TOMBE SUR UNE LIMITE DE CONTENU, JAMAIS AU MILIEU.
//
// La première version bornait à 620 px exactement, ce qui tranchait au hasard :
// au milieu d’un paragraphe, entre deux lignes d’un tableau, en travers d’une
// légende. Il fallait alors un dégradé pour dissimuler la plaie — le flou n’était
// pas un effet, c’était un pansement. On mesure donc les blocs de premier rang du
// chapitre et l’on s’arrête au dernier qui tient sous le seuil. La coupe devient
// une pliure nette, le dégradé disparaît, et ce que le lecteur voit avant de
// déplier est un ensemble complet plutôt qu’un fragment estompé.
//
// Le chapitre ne se replie que s’il en vaut la peine : sous le seuil, il reste
// ouvert et le bouton n’apparaît pas. Rien n’est caché à l’impression, ni aux
...
const Chapitre = ({ children, lang = 'fr' }) => {
  const L = faireL(lang);
  const ref = useRef(null);
  const [pliure, setPliure] = useState(null);   // hauteur de la coupe, ou null
  const [reste, setReste] = useState(0);        // blocs encore repliés
  const [ouvert, setOuvert] = useState(false);
  ...
  useLayoutEffect(() => {
    ...
    // OÙ COUPER. On relevè toutes les limites de bloc du chapitre — sur trois
    // niveaux de profondeur, ce qui suffit à descendre de la <section> à ses
    // paragraphes — et l’on retient celle qui tombe le plus près du seuil.
    //
    // Deux garde-fous, parce qu’une pliure mal placée est pire que pas de pliure :
    // il faut qu’il reste assez à voir (sinon on masque l’analyse au lieu de la
    // plier) et assez à cacher (sinon le bouton ne gagne rien).
    const MINI_VU = 320;
    const MINI_CACHE = 260;

    const limites = (racine) => {
      const haut = racine.getBoundingClientRect().top;
      const trouvees = [];
      const descendre = (n, profondeur) => {
        for (const c of n.children) {
          if (c.offsetHeight < 24) continue;
          trouvees.push(c.getBoundingClientRect().bottom - haut);
          if (profondeur < 3) descendre(c, profondeur + 1);
        }
      };
      descendre(racine, 1);
      return trouvees;
    ...
    // Les rectangles de tout ce qui porte du texte. Une limite de bloc peut être
    // propre pour la colonne d’où elle vient et trancher la colonne d’à côté :
    // dans une grille, la fin d’une cellule n’est la fin de rien. On refuse donc
    // toute limite qui traverse un texte, d’où qu’il vienne.
    const textes = (racine) => {
      const haut = racine.getBoundingClientRect().top;
      const out = [];
      for (const e of racine.querySelectorAll('*')) {
        ...
        for (const n of e.childNodes) {
          if (n.nodeType === 3 && n.textContent.trim().length > 1) { porte = true; break; }
        }
        if (!porte) continue;
        const r = e.getBoundingClientRect();
        out.push([r.top - haut, r.bottom - haut]);
      }
      return out;
    };
    ...
    const mesurer = () => {
      ...
      if (total <= SEUIL_CHAPITRE + 120) { setPliure(null); setReste(0); return; }

      const zones = textes(el);
      const traverse = (y) => zones.some(([t, b]) => t < y - 2 && b > y + 2);

      const bonne = limites(el)
        .filter(y => y >= MINI_VU && total - y >= MINI_CACHE && !traverse(y))
        .sort((a, b) => Math.abs(a - SEUIL_CHAPITRE) - Math.abs(b - SEUIL_CHAPITRE))[0];

      if (bonne === undefined) { setPliure(null); setReste(0); return; }
      setPliure(Math.round(bonne));
      setReste(1);
    };

    mesurer();
    // Le contenu bouge avec la largeur de la fenêtre et le chargement des
    // polices : une mesure unique au montage se tromperait à la première
    // rotation d’écran.
  ...
  const long = pliure !== null && reste > 0;
  const replie = long && !ouvert;

  return (
    <div className="chapitre" data-replie={replie ? 'true' : 'false'}>
      <div ref={ref} className="chapitre-corps"
           style={replie ? { maxHeight: `${pliure}px` } : undefined}>{children}</div>
      {long && (
        <button type="button" className="chapitre-pliure" onClick={() => setOuvert(o => !o)} aria-expanded={ouvert}>
          <span className="chapitre-pliure-mot">
            {ouvert ? L('Replier', 'Collapse', { ar: 'طيّ' }) : L('Lire la suite', 'Read on', { ar: 'تابع القراءة' })}
          </span>
          <SceauMarque className="w-4 h-4 shrink-0" />
        </button>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <SceneFlux lang={lang}>
        <div className="max-w-3xl">
          ...
          <span className="surtitre surtitre--clair">
            {L('Atlas — les mobilités africaines, question par question',
               'Atlas — African mobility, question by question',
               { ar: 'أطلس — التنقلات الأفريقية، سؤالاً بسؤال' })}
          </span>
          ...
          <nav aria-label={L('Couches de la carte', 'Map layers', { ar: 'طبقات الخريطة' })} className="mt-0">
            ...
            <p className="scene-flux-mode">
              {L(`${COUCHES_ATLAS.length} questions posées au continent — survolez pour un chiffre, cliquez pour le pays.`,
                 `${COUCHES_ATLAS.length} questions put to the continent — hover for a figure, click for the country.`,
      ...
      <div className="atlas-scene" data-panneau={paysOuvert ? 'ouvert' : 'ferme'}>
        <div className="atlas-carte">
          <AfricaChoropleth
            ...
            onSelect={(id) => {
              // OUVRIR UN PAYS, C'EST AUSSI ENTRER DANS SA RÉGION. La fiche
              // répond « ce pays » ; la question suivante est toujours « par
              // rapport à quoi ». En calant le cadre sur la région du pays, la
              // carte le montre parmi ses voisins et le bloc d'ensemble, en bas
              // de page, bascule sur la perspective correspondante — cliquer le
              // Maroc donne la fiche du Maroc ET l'Afrique méditerranéenne.
              // `countryRegionMap` porte déjà l'association ; elle n'était pas
              // branchée.
              const ouvre = id !== paysOuvert;
              setPaysOuvert(ouvre ? id : null);
              if (ouvre) {
                const region = countryRegionMap[id];
                if (region) setActiveSubRegion(region);
              }
            }}
          />
        </div>
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-4 pt-1 border-t" style={{ borderColor: 'var(--rule)' }}>
        {/* La consigne de lecture et la provenance en UNE note, pas en deux
            paragraphes. C’est la forme `.note-source` employée partout
            ailleurs sur le site : un filet fin, un corps de note, la couleur
            d’étiquette. Elles disaient comment lire un aplat et d’où il vient
            — cela se met en note, pas en texte courant. */}
        <p className="note-source" style={{ marginTop: 0, borderTop: 0, paddingTop: 0 }}>
          {tr(plain, lang)}
          {hint && (
            <>
              {' — '}
              {/* L espace vit dans le TEXTE, pas seulement dans la marge : sans lui,
                  un copier-coller rend « SourceMigrants ». Le site releve deja ce defaut
                  ailleurs, sur « 29millions ». */}
              <span className="surtitre">{L('Source', 'Source', { ar: 'المصدر' })}</span>
              {' : '}
              {tr(hint, lang)}
            </>
          )}
          ...
          <div className="px-6 md:px-8 py-5 border-t border-slate-100">
            <div className="flex justify-between text-[11px] font-bold mb-2">
              ...
              <span style={{ color: 'var(--label)' }}>
                {tr(display.distribution[1].label, lang)} ({formatNumber(display.distribution[1].value, lang)} %)
              </span>
            </div>
...
// Les séries continentales, calculées une fois : le rang d’un pays se lit contre
// les cinquante-trois autres, pas contre une moyenne. Les entités sans donnée sur
// un indicateur sortent de la série de cet indicateur seulement — un pays qui n’a
// pas de série de transferts garde son rang démographique.
const seriesContinentales = (() => {
  const tous = Object.values(countryData).flat();
  return {
    stock:        tous.map(c => c.stock),
    part:         tous.map(c => c.evolution),
    female:       tous.map(c => c.female),
    retention:    tous.map(c => c.retention),
    remittances:  tous.map(c => c.remittances),
    labour:       tous.map(c => c.labour_participation),
  };
})();
...
const computeRegionAggregate = (countries) => {
  ...
  // PAS DE RÉTENTION PONDÉRÉE. Le poids disponible est `stock`, le nombre de
  // migrants PRÉSENTS dans le pays ; la rétention est un rapport sur les
  // PARTANTS. Pondérer l’un par l’autre est une erreur de catégorie, et la
  // base ne porte pas les stocks d’émigrés qui permettraient de faire
  // autrement. La grandeur n’est donc pas calculable ici : on ne la calcule
  // pas. Le continent, lui, a sa valeur vérifiée dans `INTRA_AFRICAIN_UA`.
...
const homeCards = [
  ...
  { id: 'data', icon: BarChart3, label: { fr: 'Données', en: 'Data' },
  ...
  { id: 'about', icon: BookOpen, label: { fr: 'À propos & méthode', en: 'About & method' },
    desc: { fr: "{count} sources vérifiées et un glossaire de {notions} notions, définies d’abord par l’instrument africain.",
            en: "{count} verified sources and a glossary of {notions} terms, defined first by the African instrument." } },
];
...
const TabHome = ({ text, lang, setActiveTab }) => {
  ...
  // Compte du glossaire : il etait ecrit en dur — et faux — dans la carte Ressources.
  const totalNotions = glossaryData.reduce((sum, g) => sum + g.terms.length, 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Reveal delay={40} className="note-cadrage bg-white rounded-xl border border-slate-200 shadow-sm p-6 md:p-10">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-4">
          ...
          <span className="surtitre inline-flex items-center gap-1.5">
        ...
        {/* Pas de pliure ici. Cette note pose le cadre scientifique du site ;
            la replier ferait de l’argument une option, et le lecteur
            arriverait sur les chiffres sans savoir d’où ils sont lus. */}
    ...
    <article className="bg-white border border-slate-200 break-inside-avoid">
      ...
      <header className="px-6 md:px-8 pt-6 pb-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="surtitre">{tr(fiche.category, lang)}</span>
        </div>

        <blockquote className="citation-examinee font-serif font-bold text-xl md:text-2xl leading-snug text-slate-900">
          {tr(fiche.narrative, lang)}
        </blockquote>

        <div className="flex items-center gap-3 mt-5 pt-4" style={{ borderTop: '1px solid var(--rule)' }}>
          <RobustnessMeter level={fiche.confidence_level} />
          <span className="surtitre inline-flex items-center gap-1.5" style={{ color: t.color }}>
...
const TabEvidenceCheck = ({ text, lang, exportEvidenceCSV }) => {
  ...
  const results = useMemo(() => {
    ...
    return evidenceCheckData
      ...
      .filter(f => !q
        || sansAccents(tr(f.narrative, lang)).includes(sansAccents(q))
        || sansAccents(tr(f.reality, lang)).includes(sansAccents(q))
        || sansAccents(tr(f.category, lang)).includes(sansAccents(q)))
      .sort((a, b) => {
        if (sortMode === 'theme') {
  ...
  // Le dossier n’est jamais vide : la premiere fiche du rail filtre est
  // ouverte d’office, a toutes les largeurs. Le visiteur arrive donc sur une
  // lecture, et non sur une invitation a choisir.
  useEffect(() => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* ------- Ce que la note évalue -------
          La portée vient AVANT la première affirmation : sans elle, le lecteur
          prend une sélection d'auteur pour un échantillon, et la jauge pour un
          jugement sur l'énoncé plutôt que sur la preuve disponible. */}
      <div>
        <h2 className="text-xl md:text-2xl font-serif font-bold text-slate-900 mb-2">{L("Ce que la note évalue, et ce qu’elle n’évalue pas", "What the rating assesses, and what it does not")}</h2>
      ...
      <div className="bg-white border border-slate-200 print:hidden mt-2">
        <div className="flex flex-col md:flex-row md:items-center gap-3 p-3 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="surtitre whitespace-nowrap">
      ...
      {results.length === 0 ? (
        ...
        {/* ------- Le dossier, pleine colonne -------
            Le rail au-dessus fait le parcours ; le dossier n’a plus a partager
            la largeur avec un registre qui disait la meme chose. Il prend donc
            toute la colonne : la fiche est plus large, et descend d’autant
            moins. */}
        <div className="print:hidden">
          {selected ? (
            <div key={selected.id} className="fiche-entree">
              <EvidenceDossier fiche={selected} lang={lang} onBack={null} showBack={false} />
            </div>
          ) : (
            <div className="etat-vide">
              <BookOpen className="w-6 h-6" style={{ color: 'var(--rule-strong)' }} aria-hidden="true" />
              <Prose lang={lang}>{L("Choisissez une affirmation dans le rail ci-dessus.", "Pick a claim from the rail above.")}</Prose>
            </div>
          )}
        </div>
        </>
...
const AuAgencyCard = ({ acronym, fullName, seat, founded, children, source, lang }) => (
  <article className="bg-white border border-slate-200 p-5 flex flex-col h-full lift">
    ...
    <span className="surtitre">{seat}</span>
      ...
      <section className="bg-white" style={{ borderStyle: 'solid', borderColor: 'var(--rule)', borderWidth: 1, borderTopWidth: 2, borderTopColor: 'var(--accent-2)' }}>
        ...
        <div className="px-6 md:px-8 py-6 space-y-6 text-sm text-slate-700 leading-relaxed">
          ...
          <div>
            ...
            <div className="overflow-x-auto border border-slate-200 p-3" style={{ backgroundColor: 'var(--paper-raised)' }}>
              ...
              <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto"
                ...
                {[0, 25, 50, 75, 100].map(v => (
                  <g key={v}>
                    <text x={PAD - 6} y={y(v) + 3} textAnchor="end" fontSize="13" fill="var(--label)">{v}</text>
                    <line x1={PAD - 3} y1={y(v)} x2={PAD} y2={y(v)} stroke="var(--rule-strong)" />
                  </g>
                ))}
                <text x={PAD} y={H - 12} fontSize="13" fill="var(--label)">
                  {L('gouvernance : rang 54 (moins bon)', 'governance: rank 54 (worst)')}
                </text>
                <text x={W - 14} y={H - 12} textAnchor="end" fontSize="13" fill="var(--label)">
                  {L('rang 1 (meilleur)', 'rank 1 (best)')}
                </text>
                <text x={10} y={26} fontSize="13" fill="var(--label)">{L('AVOI', 'AVOI')}</text>
                {d.pts.map(p => (
                ...
                <table className="w-full text-[12px]">
                  ...
                  <thead>
                    <tr style={{ backgroundColor: 'var(--paper-sunk)' }}>
                      ...
                      <th scope="col" className="px-2 py-2 font-semibold text-slate-700 text-end">{L('Gouvernance', 'Governance')}</th>
                      <th scope="col" className="px-2 py-2 font-semibold text-slate-700 text-end">AVOI</th>
                      <th scope="col" className="px-3 py-2 font-semibold text-slate-700 text-end">{L('Ancrage', 'Anchoring')}</th>
                    </tr>
                  </thead>
...
const AnchoringMatrix = ({ lang }) => {
  ...
  // LE TRI PAR COLONNE. Deux tris — par ancrage, par nom — répondaient à deux
  // questions ; la matrice en porte huit. « Qui a ratifié Kampala ? » demandait de
  // parcourir cinquante-quatre lignes à l'œil en suivant une colonne de points.
  // Chaque en-tête devient donc un tri : les ratifiants remontent, et le compte
  // de la colonne est déjà là, sous le libellé.
  const sorted = useMemo(() => {
    const c = [...rows];
    if (sortBy === 'score') return c.sort((a, b) => b.score - a.score || a.name.localeCompare(b.name));
    if (sortBy === 'name') return c.sort((a, b) => a.name.localeCompare(b.name));
    return c.sort((a, b) =>
      (b.t[sortBy] ? 1 : 0) - (a.t[sortBy] ? 1 : 0) || a.name.localeCompare(b.name));
  }, [rows, sortBy]);

  // Ce que la colonne triée annonce aux outils d'assistance. `aria-sort` ne vaut
  // que sur l'en-tête qui commande le tri, jamais sur les autres.
  const ordreDe = (cle) => (sortBy === cle ? 'descending' : 'none');
    ...
    <Chapitre lang={lang}>
      <section className="bg-white" style={{ borderStyle: 'solid', borderColor: 'var(--rule)', borderWidth: 1, borderTopWidth: 2, borderTopColor: 'var(--accent)' }}>
        <div className="px-6 md:px-8 pt-6 pb-5 border-b border-slate-200">
          ...
          <h3 className="font-serif font-bold text-xl md:text-2xl text-slate-900 leading-snug">
            {L("L’engagement décroche dès qu’il devient contraignant",
        ...
        <div className="px-6 md:px-8 py-6 space-y-6 text-sm text-slate-700 leading-relaxed">
          ...
          <div className="aparte aparte--contre">
            <p className="surtitre" style={{ color: 'var(--bad)' }}>
            ...
            <p className="text-[11px] mt-3 leading-relaxed" style={{ color: 'var(--ink-mute)' }}>
              <span className="surtitre">{L('Concernés :', 'Concerned:')}</span>{' '}
          ...
          <div>
            ...
            <div className="overflow-x-auto border border-slate-200">
              <table className="w-full text-[12px]">
                <caption className="table-titre">
                  {L('Ratification des six instruments continentaux, pays par pays. Cliquez un en-tête pour trier.',
                     'Ratification of the six continental instruments, country by country. Click a header to sort.')}
                </caption>
                <thead>
                  <tr style={{ backgroundColor: 'var(--paper-sunk)' }}>
                    <th scope="col" aria-sort={ordreDe('name')}
                        className="text-start font-semibold px-3 py-2 text-slate-700 whitespace-nowrap">
                      <button type="button" className="tri-col" onClick={() => setSortBy('name')}>{L('Pays', 'Country')}</button>
                    </th>
                    {ANCHOR_INSTRUMENTS.map(i => (
                      <th scope="col" key={i.key} aria-sort={ordreDe(i.key)}
                          className="px-2 py-2 font-semibold text-slate-700 text-center whitespace-nowrap">
                        <button type="button" className="tri-col" onClick={() => setSortBy(i.key)}
                                title={`${tr(i.full, lang)} — ${totals.find(t => t.key === i.key)?.n || 0}/${total}`}>
                          {tr(i.short, lang)}
                          <em className="tri-compte">{totals.find(t => t.key === i.key)?.n || 0}</em>
                        </button>
                      </th>
                    ))}
                    <th scope="col" aria-sort={ordreDe('score')}
                        className="px-3 py-2 font-semibold text-slate-700 text-end whitespace-nowrap">
                      <button type="button" className="tri-col" onClick={() => setSortBy('score')}>{L('Score', 'Score')}</button>
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {sorted.map(r => (
                    <tr key={r.iso2 || r.name} className="figure-row">
                      <th scope="row" className="px-3 py-1.5 text-slate-800 whitespace-nowrap text-start font-normal">{r.name}</th>
                      {ANCHOR_INSTRUMENTS.map(i => (
...
const AfricanCounterpoint = ({ kicker, title, lang, sources = [], accent = 'var(--accent-deep)', children }) => (
  ...
  >
    <div className="px-6 md:px-8 pt-6 pb-5 border-b border-slate-200">
      <span className="surtitre" style={{ color: accent }}>
        {kicker}
      </span>
...
// L'AFRIQUE COMPAREE
//
// La plateforme mesure l'Afrique pour elle-meme, et c'est bien l'ordre : on
// etablit d'abord ce que le continent est, puis on le situe. Sans ce second
// temps, chaque mesure reste invérifiable pour le lecteur — « 64 % des partants
// restent dans la region » se lit comme beaucoup ou comme peu selon ce qu'on
// croit savoir, et c'est exactement la ou les recits s'installent.
//
// La comparaison n'est donc pas un supplement : c'est ce qui rend un chiffre
// africain opposable. Elle sert aussi bien quand elle place l'Afrique du bon
// cote de l'etalon que quand elle l'en ecarte, et le composant ne trie pas
// selon ce qui arrange.
//
// LA RESERVE DE PERIMETRE precede toujours les chiffres. UN DESA raisonne sur
// les regions des objectifs de developpement durable : l'Afrique du Nord y est
// rangee avec l'Asie de l'Ouest, dans un ensemble qui va du Maroc a l'Iran.
// Aucune ligne ne decrit donc les 54 Etats de l'Union africaine, et additionner
// ces parts a celles de la base pays serait une faute. On l'ecrit avant, pas en
// note de bas de page.
const nomRegion = (cle, lang) => tr(REGIONS_DESA[cle] || { fr: cle, en: cle }, lang);

const CompareRegions = ({ lignes, champ, lang, repere = null, unite = '%',
                         relief = 'ssa', etalon = 'monde', max = 100, titre }) => {
  const L = faireL(lang);
  const triees = [...lignes].sort((a, b) => (b[champ] ?? -1) - (a[champ] ?? -1));
  const pourcent = (v) => `${Math.max(0, Math.min(100, (v / max) * 100))}%`;
  return (
    <div>
      {titre && <p className="surtitre" style={{ color: 'var(--label)' }}>{titre}</p>}
      <div role="list">
        {triees.map((l) => {
          const v = l[champ];
          const estRelief = l.region === relief;
          const estEtalon = l.region === etalon;
          return (
            <div key={l.region} role="listitem" className="compare-ligne"
                 data-relief={estRelief ? 'true' : undefined}
                 data-etalon={estEtalon ? 'true' : undefined}>
              <span className="compare-nom">{nomRegion(l.region, lang)}</span>
              <span className="compare-jauge" aria-hidden="true"
...
// La reserve, ecrite une fois et posee partout ou ces chiffres paraissent.
const ReservePerimetre = ({ lang }) => {
  const L = faireL(lang);
  return (
    <p className="compare-reserve">
      {L("Découpage régional des Nations unies, et non celui de l’Union africaine : l’Afrique subsaharienne y compte 51 pays, tandis que l’Afrique du Nord est rangée avec l’Asie de l’Ouest dans un ensemble de 25 pays qui va du Maroc à l’Iran. Ces parts ne décrivent donc pas les 54 États de l’Union africaine et ne s’additionnent pas à celles de la base pays.",
        "United Nations regional grouping, not the African Union’s: Sub-Saharan Africa here counts 51 countries, while Northern Africa is grouped with Western Asia in a 25-country set running from Morocco to Iran. These shares therefore do not describe the African Union’s 54 member states, and cannot be added to those of the country base.")}
    </p>
  );
};
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Reveal delay={20} className="bg-white border border-slate-200 p-8 md:p-10">
        ...
        <div className="space-y-4 text-sm text-slate-700 leading-relaxed max-w-4xl text-justify">
          <Prose className="lede" lang={lang}>{L(
              "Parler de « migration » pour désigner l’ensemble des mobilités africaines efface la distinction qui compte le plus en droit, celle qui sépare la personne qui choisit de partir de celle qui y est contrainte. Or, parmi les contraintes, le droit distingue encore selon qu’une frontière internationale a été franchie ou non. De ce seul franchissement dépendent le statut de la personne, l’institution compétente pour la protéger, le financement mobilisable et jusqu’à sa visibilité dans les statistiques.",
              'Using "migration" for the whole of African mobility erases the distinction that matters most in law, the one separating the person who chooses to leave from the person compelled to. And among the compelled, law distinguishes further according to whether an international border has been crossed. On that crossing alone depend the person’s status, the institution competent to protect them, the funding that can be mobilised, and even their visibility in the statistics.'
            )}</Prose>
        </div>
      </Reveal>
      ...
      <Reveal delay={15}>
        <CarteSection
          ...
          plain={{
            fr: "Plus la teinte est dense, plus le pays compte de personnes déplacées. Elle se concentre sur une poignée d’États : le total continental n’est pas réparti, il est porté.",
            en: 'The denser the shade, the more displaced people the country holds. It concentrates on a handful of states: the continental total is not spread, it is carried.',
          }}
      ...
      <Reveal delay={30} className="bg-white border border-slate-200 p-8 md:p-10">
        ...
        <p className="surtitre" style={{ color: 'var(--warn-ink)' }}>
        ...
        <p className="surtitre mt-8" style={{ color: 'var(--accent-2)' }}>
        ...
        <p className="surtitre mt-8" style={{ color: 'var(--ok)' }}>
        ...
        <p className="surtitre mt-8" style={{ color: 'var(--bad)' }}>
        ...
        <div className="mt-8 pt-6" style={{ borderTop: '1px solid var(--rule)' }}>
          ...
          <Prose className="text-[13px] text-slate-600 leading-relaxed text-justify mt-4" lang={lang}>{L(
              "En dix ans, le nombre de personnes déplacées à l’intérieur de leur propre pays et suivies par le HCR a plus que triplé sur le continent, tandis que celui des réfugiés, qui franchissent une frontière, a un peu plus que doublé. La mobilité forcée africaine ne s’internationalise donc pas : elle s’intensifie à l’intérieur des frontières nationales. Or c’est précisément cette population que les statistiques migratoires n’enregistrent pas, faute de franchissement à comptabiliser, et que le seul traité conçu pour la protéger n’atteint qu’à proportion des ratifications obtenues.",
              "Over ten years the number of people displaced inside their own country and monitored by UNHCR has more than tripled on the continent, while the number of refugees, who cross a border, has slightly more than doubled. African forced mobility is therefore not internationalising: it is intensifying within national borders. Yet this is precisely the population that migration statistics do not record, there being no crossing to count, and that the only treaty designed to protect it reaches only in proportion to the ratifications secured."
            )}</Prose>
        </div>
...
const TabMobilites = ({ text, lang, volet, setVolet }) => {
  ...
  const bascule = (
    <div className="mb-2 -mt-2">
      {/* La these de la section, au-dessus de la bascule. Sans elle les quatre
          volets se lisent comme un sommaire — quatre sujets qui cohabitent —
          alors qu'ils demontrent le meme constat sur quatre terrains. */}
      <Prose className="text-sm leading-relaxed max-w-4xl mb-3" style={{ color: 'var(--ink-soft)' }} lang={lang}>{L(
          "Ces quatre formes de mobilité ont un constat en commun\u202f: elles sont majoritairement internes aux pays ou intra-africaines. Or ce sont précisément celles que l’appareil statistique saisit le moins bien — un déplacement qui ne franchit aucune frontière ne produit aucune entrée à compter.",
          "These four forms of mobility share one finding: they are predominantly internal to countries or intra-African. Yet they are precisely the ones statistical systems capture least well — movement that crosses no border produces no entry to count."
        )}</Prose>
    <nav className="bascule-groupe flex flex-wrap gap-2" aria-label={L('Volets de la section', 'Section panes', { ar: 'أقسام الصفحة' })}>
        {volets.map(v => (
          <button
            key={v.cle}
            type="button"
  ...
  // Un chapeau par volet, posé sous le bandeau que chaque volet affiche.
  // Corridors fait exception : il porte le sien plus bas, au plus près des
  // deux objets qu’il distingue, et n’en reçoit donc pas ici — il en
  // affichait deux.
  const entete = (
    <>
      {bascule}
      {volet === 'climat' ? (
        <Reperes
          ...
          notions={[
            ...
            { mot: { fr: 'Populations immobilisées', en: 'Trapped populations' },
              sens: { fr: "Des populations exposées à un risque qu’elles ne peuvent fuir, faute des ressources qu’exige un départ. Ce sont les plus pauvres qui restent là où le risque est le plus fort, et parce qu’aucun mouvement n’est enregistré, aucune statistique de mobilité ne les fait apparaître.",
                      en: 'Populations exposed to a risk they cannot flee, lacking the resources departure demands. It is the poorest who stay where risk is greatest, and because no movement is recorded, no mobility statistic makes them visible.' } },
          ]}
        ...
        <Reperes
          lang={lang}
          titre={{ fr: "Ce qui fait d’un départ une migration de travail",
                   en: 'What makes a departure a labour migration',
                   ar: 'ما الذي يجعل من المغادرة هجرة عمل' }}
          chapeau={{
            fr: "Ce volet mesure des personnes employées hors de leur pays, et l’écart entre le droit qui les protège sur le papier et celui qu’elles peuvent effectivement invoquer. Cet écart ne se lit qu’à condition de distinguer quatre notions que l’usage courant confond, et dont chacune commande une partie du raisonnement.",
            en: 'This section measures people employed outside their own country, and the gap between the rights that protect them on paper and those they can actually invoke. That gap can be read only by distinguishing four notions that common usage runs together, each of which governs one part of the argument.'
          }}
          notions={[
            { mot: { fr: 'Travailleur migrant', en: 'Migrant worker' },
              sens: { fr: "Une personne qui migre en vue d’occuper un emploi dans un pays dont elle n’est pas ressortissante. C’est la définition des conventions n° 97 (1949) et n° 143 (1975) de l’OIT ; elle ne dit rien de la régularité du séjour, et n’exclut donc pas l’emploi informel.",
                      en: 'A person who migrates with a view to being employed in a country of which they are not a national. This is the definition of ILO Conventions No. 97 (1949) and No. 143 (1975); it says nothing about the regularity of stay, and therefore does not exclude informal employment.' } },
            { mot: { fr: 'Emploi informel', en: 'Informal employment' },
              sens: { fr: "Un emploi sans contrat écrit, sans couverture sociale, ou dans une unité non enregistrée. C’est là que se trouve l’essentiel de l’emploi africain — et c’est ce que les enquêtes atteignent le plus mal, migrants compris.",
                      en: 'Work without a written contract, without social coverage, or in an unregistered unit. This is where most African employment sits — and what surveys reach least well, migrants included.' } },
            { mot: { fr: 'Convention ratifiée', en: 'Ratified convention' },
              sens: { fr: "Une convention de l’OIT n’oblige un État qu’après ratification, enregistrée dans la base NORMLEX. Un droit reconnu par un texte non ratifié n’est pas opposable : c’est l’écart que ce volet mesure, pays par pays.",
                      en: 'An ILO convention binds a state only once ratified, as recorded in the NORMLEX database. A right recognised by an unratified text cannot be invoked: that is the gap this section measures, country by country.' } },
            { mot: { fr: 'Gaspillage de compétences', en: 'Brain waste' },
              sens: { fr: "Un migrant hautement qualifié occupant un emploi sans rapport avec ses diplômes, faute de reconnaissance de ses qualifications. La perte est double : pour la personne, et pour les deux pays.",
                      en: 'A highly qualified migrant holding a job unrelated to their credentials, for want of recognition of their qualifications. The loss is twofold: for the person, and for both countries.' } },
          ]}
          pied={{
            fr: "Sources : OIT, conventions n° 97 et n° 143 et base NORMLEX ; Glossaire de la plateforme pour le gaspillage de compétences.",
            en: 'Sources: ILO Conventions No. 97 and No. 143 and the NORMLEX database; this platform’s Glossary for brain waste.'
          }}
        />
      ) : volet === 'corridors' ? null : (
      <Reperes
        lang={lang}
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Reveal delay={25}>
        <AfricanCounterpoint
          ...
          title={L(
            "Deux éditions du même rapport s’accordent sur le fond et divergent sur les chiffres",
            "Two editions of the same report agree in substance and diverge on the figures"
          )}
          sources={[
            { label: L(
            ...
            { label: L(
                "Commission de l’Union africaine, OIT et OIM (programme JLMP, appui technique de Statistics Sweden) — « Report on Labour Migration Statistics in Africa », 4e édition (données 2022), lancée le 31 juillet 2026 au Comité technique spécialisé développement social, travail et emploi, à Windhoek.",
                "African Union Commission, ILO and IOM (JLMP programme, technical support from Statistics Sweden) — Report on Labour Migration Statistics in Africa, 4th edition (2022 data), launched on 31 July 2026 at the Specialised Technical Committee on Social Development, Labour and Employment, in Windhoek."
              ), url: "https://au.int/sites/default/files/4th_Edi_LMSRA_EN_WEB_20260626.pdf" },
          ]}
        >
          <Prose className="text-justify" lang={lang}>{L(
              "Le rapport continental sur les statistiques de migration de travail en est à sa quatrième édition. Lues ensemble, les deux dernières s’accordent sur le fond — la mobilité intra-africaine est d’abord une mobilité de travail — et divergent sur les chiffres. Cette divergence n’affaiblit pas le constat : elle montre qu’une série statistique se révise à mesure que les États déclarants s’ajoutent, et elle permet d’observer directement l’effet de l’harmonisation sur ce que le continent sait de lui-même.",
              "The continental report on labour migration statistics is now in its fourth edition. Read together, the last two agree in substance — intra-African mobility is above all labour mobility — and diverge on the figures. That divergence does not weaken the finding: it shows that a statistical series is revised as reporting states are added, and it makes the effect of harmonisation on what the continent knows about itself directly observable."
            )}</Prose>
          ...
          <div className="bg-white border border-slate-200 overflow-x-auto">
            <table className="w-full text-[13px]">
              <caption className="table-titre">
                {L("Rapport continental sur les statistiques de migration de travail : 3e édition (données 2019) et 4e édition (données 2022).",
                   "Continental report on labour migration statistics: 3rd edition (2019 data) and 4th edition (2022 data).")}
              </caption>
              <thead>
                <tr style={{ backgroundColor: 'var(--paper-sunk)' }}>
                  <th scope="col" className="text-start font-semibold px-4 py-2.5 text-slate-700">{L("Indicateur", "Indicator")}</th>
                  <th scope="col" className="text-end font-semibold px-4 py-2.5 text-slate-700 whitespace-nowrap">{L("3e éd. — 2019", "3rd ed. — 2019")}</th>
                  <th scope="col" className="text-end font-semibold px-4 py-2.5 text-slate-700 whitespace-nowrap">{L("4e éd. — 2022", "4th ed. — 2022")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {(tr({ fr: [
          ...
          <div className="aparte">
            ...
            <Prose className="text-[13px] text-slate-600 leading-relaxed text-justify" lang={lang}>{L(
                "Le point de départ lui-même a bougé : 9,5 millions de travailleurs migrants en 2010 selon la 3e édition, 9,3 millions selon la 4e.\n\nIl s’agit d’une révision : méthodologie affinée, davantage d’États déclarants, séries recalculées. La 4e édition documente d’ailleurs le décrochage de 2020 (croissance tombée à 0,67 %, effet de la pandémie sur la mobilité) que la précédente ne pouvait pas voir.\n\nUne plateforme de données doit montrer ces révisions plutôt que de retenir le chiffre le plus commode : c’est précisément l’objet de l’harmonisation que vise SHaSA.",
                "The starting point itself moved: 9.5 million migrant workers in 2010 according to the 3rd edition, 9.3 million according to the 4th.\n\nThis is a revision: refined methodology, more reporting states, recomputed series. The 4th edition also documents the 2020 break (growth down to 0.67%, the pandemic’s effect on mobility) that the previous one could not see.\n\nA data platform should show such revisions rather than retain the most convenient figure: that is exactly what SHaSA’s harmonisation is for."
              )}</Prose>
          </div>
          ...
          <div>
            ...
            <Prose className="text-[13px] text-slate-600 leading-relaxed text-justify" lang={lang}>{L(
                "Le chiffre vaut pour ces dix pays et ne vaut pas pour le continent, puisque rien ne garantit que les quarante-quatre absents présentent la même structure d’emploi. Ce que révèle cet écart n’est pas une carence de production : les données brutes existent dans les appareils nationaux. Il porte sur la chaîne qui les fait remonter et les rend comparables (Ben Mokhtar, 2026).",
                "The figure holds for these ten countries and does not hold for the continent, since nothing guarantees that the forty-four absent ones share the same employment structure. What this gap reveals is not a failure of production: the raw data exist within national systems. It concerns the chain that reports them upward and makes them comparable (Ben Mokhtar, 2026)."
              )}</Prose>
          </div>
          ...
          <div>
            ...
            <Prose className="text-[13px] text-slate-600 leading-relaxed text-justify" lang={lang}>{L(
                "La 4e édition établit elle-même le lien. L’augmentation du nombre de travailleurs migrants « pourrait être liée à l’assouplissement des restrictions migratoires et à la mise en œuvre des dispositions de libre circulation entre pays africains ».\n\nElle en donne un cas mesuré — au sein de la Communauté d’Afrique de l’Est, les travailleurs migrants passent de 1,14 million en 2008 à 2,69 millions en 2019. Et elle constate que l’Ouest, l’Est et le Sud, où les protocoles CEDEAO, CAE et SADC fonctionnent, concentrent 77,3 % des migrants internationaux du continent.",
                "The 4th edition draws the link itself: the rise in migrant worker numbers \"might be linked to relaxed migration restrictions and to the implementation of free movement provisions between African countries\".\n\nIt gives one measured case — within the East African Community, migrant workers rise from 1.14 million in 2008 to 2.69 million in 2019. And it notes that West, East and Southern Africa, where the ECOWAS, EAC and SADC protocols operate, concentrate 77.3% of the continent’s international migrants."
              )}</Prose>
      ...
      <Reveal delay={35}>
        <Chapitre lang={lang}>
          <section className="bg-white" style={{ borderStyle: 'solid', borderColor: 'var(--rule)', borderWidth: 1, borderTopWidth: 2, borderTopColor: 'var(--warn-ink)' }}>
            <div className="px-6 md:px-8 pt-6 pb-5 border-b border-slate-200">
              <span className="surtitre surtitre--reserve">
                {L('Calculé depuis la base de la plateforme', 'Computed from the platform database')}
              </span>
              <h3 className="font-serif font-bold text-xl md:text-2xl text-slate-900 leading-snug">
            ...
            <div className="px-6 md:px-8 py-6 space-y-6 text-sm text-slate-700 leading-relaxed">
              ...
              <div>
                ...
                <p className="text-[11px] mt-3 leading-relaxed" style={{ color: 'var(--label)' }}>
                  <span className="surtitre">{L('Socle complet :', 'Full floor:')}</span>{' '}
                  {ilo.complet.map(r => r.n).join(', ')}.
                </p>
              </div>
  
              <div className="aparte aparte--reserve">
                <p className="surtitre" style={{ color: 'var(--warn-ink)' }}>
              ...
              <div className="overflow-x-auto border border-slate-200">
                <table className="w-full text-[12px]">
                  ...
                  <thead>
                    <tr style={{ backgroundColor: 'var(--paper-sunk)' }}>
                      <th scope="col" className="text-start font-semibold px-3 py-2 text-slate-700">{L('Pays', 'Country')}</th>
                      <th scope="col" className="px-2 py-2 font-semibold text-slate-700 text-end">{L('Fondamentales', 'Fundamental')}</th>
                      <th scope="col" className="px-2 py-2 font-semibold text-slate-700 text-end">{L('Gouvernance', 'Governance')}</th>
                      <th scope="col" className="px-2 py-2 font-semibold text-slate-700 text-end">{L('Techniques', 'Technical')}</th>
                      <th scope="col" className="px-3 py-2 font-semibold text-slate-700 text-end">{L('Total', 'Total')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {ilo.rows.map(r => (
      ...
      <Reveal delay={45} className="bg-white border border-slate-200 p-8 md:p-10">
        ...
        <Sources lang={lang} items={[
          L("Commission de l’Union africaine, OIT et OIM (programme JLMP, appui technique de Statistics Sweden) — « Report on Labour Migration Statistics in Africa », 4e édition (données 2022), lancée le 31 juillet 2026 au Comité technique spécialisé développement social, travail et emploi, à Windhoek.",
            "African Union Commission, ILO and IOM (JLMP programme, technical support from Statistics Sweden) — Report on Labour Migration Statistics in Africa, 4th edition (2022 data), launched on 31 July 2026 at the Specialised Technical Committee on Social Development, Labour and Employment, in Windhoek."),
        ]} />
      </Reveal>
...
const TabCorridors = ({ text, lang, children }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* LA SCÈNE PLUTÔT QUE LE BANDEAU. `SceneFlux` est tracée sur
          `CORRIDORS` — la donnée même de cette section. C'est le seul
          endroit du site où cette carte illustre exactement son sujet, et
          le trait y porte déjà le constat : plein pour l'intra-africain,
          fin pour ce qui sort du continent. */}
      <SceneFlux lang={lang}>
        <div className="max-w-3xl">
          <span className="surtitre surtitre--clair">
            {text.headers.corridors.badge}
          </span>
          <h1 className="font-serif font-bold text-2xl md:text-4xl leading-[1.06]"
              style={{ color: 'var(--reserve)' }}>
            {text.headers.corridors.title}{' '}
            <span style={{ color: 'var(--accent-light)' }}>
              {text.headers.corridors.highlight}
            </span>
          </h1>
          <Prose className="mt-3 text-[15px] leading-relaxed max-w-xl"
                 style={{ color: 'var(--reserve-douce)' }} lang={lang}>
            {text.headers.corridors.desc}
          </Prose>
        </div>
      </SceneFlux>
      ...
      <Reperes
        ...
        notions={[
          { mot: { fr: 'Corridor', en: 'Corridor' },
            sens: { fr: "Un couple de pays, mesuré par le nombre de personnes nées dans le premier et résidant dans le second. La mesure est tirée des recensements : elle décrit une installation constatée à une date, et laisse hors champ les allers-retours comme les séjours qui n’ont pas abouti à une résidence.",
                    en: 'A pair of countries, measured by how many people born in the first reside in the second. The measurement comes from censuses: it describes settlement observed at a date, and leaves out round trips as well as stays that did not result in residence.' } },
        ...
        pied={{
          fr: "Un corridor documenté par les stocks se met à jour tous les deux ans et ignore les allers-retours ; une route suivie sur le terrain se met à jour tous les mois et ignore ceux qui restent. Aucune des deux ne peut donc se substituer à l’autre : la première décrit une installation, la seconde un passage, et confondre les deux revient à lire un volume là où il n’y a qu’un flux, ou l’inverse.",
          en: 'A corridor documented through stocks updates every two years and ignores round trips; a route monitored on the ground updates monthly and ignores those who stay. Neither can therefore stand in for the other: the first describes settlement, the second passage, and conflating them means reading a volume where there is only a flow, or the reverse.'
        }}
      />
      ...
      <Reveal className="bg-white border border-slate-200 overflow-hidden">
        <div className="px-6 md:px-8 pt-6 pb-4">
          <h2 className="font-serif font-bold text-xl md:text-2xl text-slate-900 leading-snug mb-2">
            {L('Un seul corridor africain pèse plus que la plupart des routes vers le Nord',
               'One African corridor alone outweighs most routes to the North',
        ...
        <div className="px-6 md:px-8 py-4 border-t border-slate-100" style={{ backgroundColor: 'var(--paper-sunk)' }}>
          <Prose className="text-[12px] leading-relaxed" style={{ color: 'var(--label)' }} lang={lang}>{L(
            "Cinq corridors seulement sont représentés ici, quand le continent en compte des dizaines. La sélection n’est pas éditoriale : ce sont les seuls pour lesquels un chiffre bilatéral publié a pu être retrouvé et vérifié. Les autres circulent, sont décrits dans la littérature et cités par les administrations, mais aucune source publique ne leur attribue de valeur qu’on puisse reprendre sans l’inventer.",
            'Only five corridors are shown here, where the continent has dozens. The selection is not editorial: these are the only ones for which a published bilateral figure could be found and verified. The others carry movement, are described in the literature and cited by administrations, but no public source assigns them a value that could be reproduced without being invented.'
          )}</Prose>
        </div>
      </Reveal>
      ...
      {/* Ce que valent 64 % : la meme mesure, pour toutes les regions. La
      ...
      <Reveal delay={40} className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
        ...
        <Prose className="text-sm text-slate-600 mb-5" lang={lang}>{L(
          "Une part intrarégionale ne signifie rien prise isolément : elle ne devient interprétable qu’une fois rapportée à ce que font les autres régions du monde. UN DESA calcule, pour chacune, la part des personnes nées dans la région qui résident dans un autre pays de cette même région ; la moyenne mondiale s’établit à 45 %, et c’est cette valeur qui sert ici de point de comparaison.",
          "An intra-regional share means nothing taken on its own: it becomes interpretable only once set against what other world regions do. UN DESA computes, for each of them, the share of people born in the region who reside in another country of that same region; the world average stands at 45 per cent, and that is the value used here as the point of comparison."
        )}</Prose>

        <ReservePerimetre lang={lang} />

        <CompareRegions
          lignes={RESTE_DANS_SA_REGION} champ="part" lang={lang}
          repere={MONDE_2024.intraRegional}
          titre={L("Part des personnes nées dans la région qui y résident encore",
                   "Share of people born in the region who still reside within it")}
        />

        <Prose className="text-sm text-slate-600 mt-5" lang={lang}>{L(
          "L’Afrique subsaharienne est à 64 %, au-dessus de la moyenne mondiale et derrière l’Europe, dont 74 % des natifs partis vivent dans un autre pays européen.\n\nCe 64 % ne décrit toutefois pas le périmètre de cette plateforme : recalculée sur les 54 États de l’Union africaine, Afrique méditerranéenne comprise, la part intra-africaine tombe à 54,4 %.\n\nÀ l’autre bout, trois régions envoient l’essentiel de leur diaspora au-delà de leurs frontières régionales : l’Asie centrale et du Sud (75 % hors région), l’Amérique du Nord (73 %) et l’Amérique latine et les Caraïbes (71 %).",
        ...
        <Sources lang={lang} items={[{ label: tr(SOURCE_MONDE.label, lang), url: SOURCE_MONDE.url }]}
                 note={L("Le seuil de 7 millions est celui que la publication indique pour l’ensemble des corridors qu’elle ne détaille pas ; elle ne donne pas la valeur du corridor Afrique subsaharienne – Europe.",
  ...
  const recsList = [
    {
      ...
      desc: {
        fr: "La CEDEAO est le bloc de référence du régime continental : c’est la seule Communauté où la libre circulation ne se déduit pas d’un objectif mais d’un droit ouvert depuis 1979, et où les administrations la traitent comme la règle plutôt que comme une faveur. La sécession de l’Alliance des États du Sahel en 2024-2025 — Mali, Burkina Faso, Niger — fragmente cet acquis sans l’abolir.",
        en: "ECOWAS is the continental regime’s benchmark bloc: it is the only Community where free movement follows not from an objective but from a right open since 1979, and where administrations treat it as the rule rather than as a favour. The 2024-2025 secession of the Alliance of Sahel States — Mali, Burkina Faso, Niger — fragments that acquis without abolishing it."
      },
      instruments: {
    ...
    },
    {
      ...
      desc: {
        fr: "La CAE fait de la mobilité une condition de crédibilité de son marché commun : sans circulation des personnes, la libre circulation des biens, des services et des capitaux resterait incomplète. Le Protocole de 2010 ouvre en conséquence le travail, la résidence et l’établissement, mais à des degrés inégaux, l’accès au marché du travail précédant partout la reconnaissance d’un droit au séjour.",
        en: "The EAC makes mobility a condition of its common market’s credibility: without the movement of people, the free movement of goods, services and capital would remain incomplete. The 2010 Protocol accordingly opens work, residence and establishment, but to unequal degrees, access to the labour market everywhere preceding recognition of a right of residence."
      },
      instruments: {
    ...
    },
    {
      ...
      desc: {
        fr: "La SADC n’a pas obtenu l’ouverture générale que son Protocole de 2005 prévoyait, l’Afrique du Sud en particulier s’y étant opposée au nom de sa souveraineté migratoire. L’intégration y progresse donc par des voies choisies : la gestion logistique des corridors commerciaux et l’attraction ciblée de qualifications, plutôt que la levée uniforme des obligations de visa.",
        en: "SADC has not achieved the general opening its 2005 Protocol provided for, South Africa in particular having opposed it in the name of its migration sovereignty. Integration therefore advances along chosen paths: logistical management of trade corridors and targeted attraction of qualifications, rather than uniform removal of visa requirements."
      },
      instruments: {
    ...
    },
    {
      ...
      desc: {
        fr: "Le COMESA réunit 21 États, de la Tunisie à l’Eswatini, dont les régimes migratoires n’ont ni la même histoire ni la même doctrine. Faute de socle commun sur lequel poser une ouverture générale, l’intégration humaine y progresse par facilités successives — visas d’affaires au premier chef — dont chacune abaisse un coût de transaction commerciale précis sans engager le régime dans son ensemble.",
        en: "COMESA brings together 21 states, from Tunisia to Eswatini, whose migration regimes share neither the same history nor the same doctrine. Lacking a common basis on which to found a general opening, human integration advances there through successive facilities — business visas first among them — each of which lowers one specific trade transaction cost without committing the regime as a whole."
      },
      instruments: {
        fr: "Protocoles de 1984 et 1998, d’application inégale. Processus consultatif : MIDCOM, qui produit l’essentiel des outils de capacitation technique du bloc.",
    ...
    },
    {
      ...
      desc: {
        fr: "Dans une Corne de l’Afrique où les conflits et les chocs climatiques déplacent plus de personnes que le travail n’en attire, l’IGAD traite la mobilité comme une question de sécurité régionale autant que de développement. Ses deux protocoles de 2020 le montrent : l’un porte sur la libre circulation des personnes, l’autre sur la transhumance pastorale transfrontalière, forme de mobilité qu’aucun autre bloc africain ne régit par un texte dédié.",
        en: "In a Horn of Africa where conflict and climate shocks displace more people than work attracts, IGAD treats mobility as a matter of regional security as much as of development. Its two 2020 protocols show this: one covers free movement of persons, the other cross-border pastoral transhumance, a form of mobility no other African bloc governs through a dedicated instrument."
      },
      instruments: {
    ...
    },
    {
      ...
      desc: {
        fr: "L’Afrique centrale fonctionne à deux vitesses. La CEMAC, noyau resserré de six États, a supprimé les visas pour quatre-vingt-dix jours et institué un passeport biométrique communautaire ; la CEEAC, qui l’englobe, n’a pas transposé cet acquis à son propre périmètre. L’enclavement de plusieurs de ses membres y ajoute une contrainte matérielle : ouvrir un droit ne suffit pas là où la route manque.",
        en: "Central Africa runs at two speeds. CEMAC, a tight core of six states, has removed visas for ninety days and established a community biometric passport; ECCAS, which contains it, has not transposed that acquis to its own perimeter. The landlocked position of several members adds a material constraint: opening a right is not enough where the road is missing."
      },
      instruments: {
    ...
    },
    {
      ...
      desc: {
        fr: "L’ambition fondatrice de 1989 est gelée : aucun sommet des chefs d’État ne s’est tenu depuis des décennies, et le différend algéro-marocain suffit à bloquer toute décision prise à l’unanimité. Ce qui subsiste d’intégration maghrébine se joue donc hors du cadre régional — dans des accords bilatéraux dispersés et dans les dispositifs d’externalisation du contrôle négociés avec l’Union européenne.",
        en: "The founding ambition of 1989 is frozen: no heads-of-state summit has been held for decades, and the Algerian-Moroccan dispute is enough to block any decision requiring unanimity. What remains of Maghreb integration therefore plays out beyond the regional framework — in scattered bilateral agreements and in the control-externalisation arrangements negotiated with the European Union."
      },
      instruments: {
    ...
    },
    {
      ...
      desc: {
        fr: "Avec 24 États membres, dont beaucoup appartiennent déjà à d’autres Communautés, la CEN-SAD fonctionne comme un forum politique et sécuritaire plus que comme un régime juridique autonome. Son traité fondateur inscrit pourtant la libre circulation des personnes parmi ses objectifs centraux, sans l’assortir d’un protocole d’application comparable à ceux de la CEDEAO ou de l’IGAD — ce qui explique qu’elle produise peu d’effets propres.",
        en: "With 24 member states, many of them already belonging to other Communities, CEN-SAD operates as a political and security forum more than as an autonomous legal regime. Its founding treaty nonetheless lists free movement of persons among its central objectives, without attaching an implementing protocol comparable to those of ECOWAS or IGAD — which explains why it produces few effects of its own."
      },
      instruments: {
            ...
            {[
              ...
              return (
                <div key={gi} className="p-4">
                  <div className="surtitre flex items-center gap-1.5">
        ...
        {activeSdgzTab === 'sdgs' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {text.sdg_section.sdg_points.map((pt, idx) => (
                <div key={idx} className="p-5 bg-white rounded-lg border border-slate-200 shadow-sm flex gap-4">
                  ...
                  <div>
                    <span className="surtitre text-blue-600">{tr({ fr: "Cible ONU", en: "UN Target" }, lang)}</span>
            ...
            >
              ...
              <Prose className="text-justify" lang={lang}>{tr({ fr: "C’est ici que le geste symbolique et l’ancrage juridique se séparent. Le passeport africain a été lancé en 2016, mais le Protocole censé le rendre opposable ne comptait, à la dernière vérification, que 4 ratifications sur 54 — très loin des 15 requises pour son entrée en vigueur. L’Agenda 2063 ne bute donc ni sur un défaut de vision ni sur l’absence d’un texte : l’un et l’autre existent, et c’est leur transposition dans les administrations nationales qui n’a pas eu lieu (Ben Mokhtar, 2026).", en: "This is where the symbolic gesture and the legal anchoring part company. The African passport was launched in 2016, yet the Protocol meant to make it enforceable stood, at the last check, at 4 ratifications out of 54 — far short of the 15 required for entry into force. Agenda 2063 therefore founders neither on a lack of vision nor on the absence of a text: both exist, and what has not happened is their transposition into national administrations (Ben Mokhtar, 2026)." }, lang)}</Prose>
            </AfricanCounterpoint>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            >
              ...
              <div>
                ...
                <Prose className="text-xs text-slate-500 mt-3 leading-relaxed text-justify" lang={lang}>{tr({ fr: "Ces dix principes forment la grille d’interprétation des 23 objectifs qui suivent, et aucun objectif ne se lit indépendamment d’eux. Comme le Pacte n’impose ni hiérarchie entre les principes ni méthode d’arbitrage lorsqu’ils entrent en tension, deux États peuvent appliquer le même objectif de bonne foi et parvenir à des politiques opposées, selon le principe auquel ils donnent le pas.", en: "These ten principles form the interpretive grid for the 23 objectives that follow, and no objective can be read independently of them. Since the Compact sets neither a hierarchy among the principles nor a method for arbitrating between them when they pull in opposite directions, two states may apply the same objective in good faith and arrive at opposed policies, depending on which principle they let prevail." }, lang)}</Prose>
              </div>
            ...
            >
              ...
              <Prose className="text-justify" lang={lang}>{tr({ fr: "Ces six domaines épousent délibérément l’architecture thématique du Pacte mondial : plutôt que de bâtir un cadre concurrent, l’Afrique occupe celui qui se négocie et y inscrit ses propres priorités. Ce qui a suivi relève de la mécanique bureaucratique plus que de la déclaration, et c’est là qu’il faut le chercher : une revue régionale africaine de la mise en œuvre du Pacte est conduite en 2021, un plan d’action continental est validé au niveau des hauts fonctionnaires en août 2024, puis adopté par le CTS-MRIDP à sa cinquième session en novembre 2025.", en: "These six domains deliberately mirror the thematic architecture of the Global Compact: rather than build a competing framework, Africa occupies the one under negotiation and writes its own priorities into it. What followed belongs to bureaucratic machinery more than to declarations, and that is where it must be looked for: an African regional review of implementation was held in 2021, a continental action plan was validated at senior-official level in August 2024, then adopted by the STC-MRIDP at its fifth session in November 2025." }, lang)}</Prose>
            </AfricanCounterpoint>
          </div>
        )}
        ...
        {activeSdgzTab === 'gcr' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            >
              ...
              <Prose className="text-justify" lang={lang}>{tr({ fr: "Cette antériorité change la façon de lire le Pacte. Le « partage équitable des charges » qu’il proclame ne décrit pas une charge à venir mais une charge déjà assumée : d’après le HCR, six pays accueillent à eux seuls plus du tiers des réfugiés du monde, dont deux africains, l’Ouganda et le Tchad. Pour le continent, l’enjeu de la négociation n’est donc pas d’adhérer à une norme venue d’ailleurs, mais d’obtenir que la norme mondiale reconnaisse cette pratique et la finance à la hauteur de ce qu’elle coûte (Ben Mokhtar, 2026).", en: "That precedence changes how the Compact reads. The \"equitable sharing of burdens\" it proclaims describes not a burden to come but one already carried: according to UNHCR, six countries alone host more than a third of the world’s refugees, two of them African, Uganda and Chad. For the continent, what is at stake in the negotiation is therefore not adherence to a norm from elsewhere, but securing that the global norm recognise this practice and fund it in proportion to its cost (Ben Mokhtar, 2026)." }, lang)}</Prose>
            </AfricanCounterpoint>
          </div>
        )}
              ...
              <div className="relative z-10">
                <div className="flex items-start gap-4 mb-2">
                  ...
                  <div className="min-w-0">
                    <span className="surtitre text-emerald-400">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="flex items-center text-lg font-serif font-bold text-slate-800 mb-6 border-b border-slate-100 pb-3">
                  ...
                  {tr({ fr: "Six textes, et une ambition en avance sur la norme mondiale", en: "Six instruments, and an ambition ahead of the global standard" }, lang)}
                </h3>
                  ...
                  {auFrameworks.map((fw, idx) => (
                    <div key={idx} className="bg-slate-50 p-6 rounded-xl border border-slate-200 flex flex-col md:flex-row gap-6">
                      ...
                      <div className="md:w-2/3 bg-white p-5 rounded-lg border border-slate-200 flex flex-col justify-center relative shadow-sm">
                        ...
                        <span className="surtitre ms-6">{tr(fw.article.ref, lang)}</span>
              ...
              <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="flex items-center text-lg font-serif font-bold text-slate-800 mb-6 border-b border-slate-100 pb-3">
                  ...
                  {tr({ fr: "Qui décide, et devant qui l’on rend compte", en: "Who decides, and to whom account is rendered" }, lang)}
                </h3>
                <div className="space-y-4">
                        ...
                        {stcSessions.map((s, idx) => (
                          <div key={idx} className="bg-white p-4 rounded-lg border border-slate-200">
                            <div className="flex flex-wrap items-baseline justify-between gap-2 mb-1.5">
                              ...
                              <span className="surtitre">{tr(s.format, lang)}</span>
                            </div>
              ...
              <CarteSection
                ...
                plain={{
                  fr: "Plus la teinte est dense, plus le pays a ratifié de ces six textes. Le blanc marque un État qui n’en a déposé aucun.",
                  en: 'The denser the shade, the more of those six instruments the country has ratified. White marks a state that has deposited none.',
                }}
              ...
              {/* ------- Quand le droit bloque -------
                  JLMP et les partenariats de competences sont les deux cartes
                  d'un meme temps de l'argument. Sans ce titre elles tombaient
                  sous « Qui decide », qui ne parle pas d'elles. */}
              <h3 className="flex items-center text-lg font-serif font-bold text-slate-800 mb-6 border-b border-slate-100 pb-3">
                <Briefcase className="w-5 h-5 me-2 text-blue-700" />
                {tr({ fr: "Quand le droit bloque, la construction se déplace", en: "When law stalls, the building shifts ground" }, lang)}
              </h3>
              ...
              <div className="bg-blue-50 p-6 md:p-7 rounded-xl border border-blue-200 shadow-sm">
                <div className="flex items-start gap-4 mb-5">
                  ...
                  <div>
                    <h3 className="font-bold text-blue-900">{tr({ fr: "Le Programme conjoint sur la migration de main-d’œuvre (JLMP)", en: "The Joint Labour Migration Programme (JLMP)" }, lang)}</h3>
                    <Prose className="text-xs text-blue-800 leading-relaxed" lang={lang}>{tr({ fr: "Le blocage du Protocole de Kigali n’a pas arrêté la construction du régime continental : elle s’est déplacée du terrain juridique vers le terrain technique. Porté conjointement par la Commission de l’Union africaine, l’OIT, l’OIM et la CEA, avec le PNUD, le JLMP met en œuvre le cinquième domaine prioritaire de la Déclaration d’Addis-Abeba sur l’emploi (2015). Il porte sur quatre chantiers — la portabilité des compétences, celle des droits à la sécurité sociale, le recrutement équitable et la protection des travailleurs migrants — qui produisent des effets sans requérir la ratification d’un texte contraignant.", en: "The deadlock on the Kigali Protocol did not halt the building of the continental regime: it shifted it from legal ground to technical ground. Jointly led by the African Union Commission, the ILO, IOM and ECA, with UNDP, the JLMP implements the fifth priority area of the 2015 Addis Ababa Declaration on Employment. It works on four fronts — portability of skills, portability of social security entitlements, fair recruitment and protection of migrant workers — which produce effects without requiring any binding instrument to be ratified." }, lang)}</Prose>
                  </div>
                </div>
                <div className="bg-white/70 p-4 rounded-lg border border-blue-100">
                  <span className="surtitre text-blue-600">{tr({ fr: "Trois phases, et une évaluation indépendante entre les deux dernières", en: "Three phases, and an independent evaluation between the last two" }, lang)}</span>
                  <Prose className="text-xs text-blue-900 leading-relaxed" lang={lang}>{tr({ fr: "Déploiement ciblé dans cinq États membres — Cameroun, Côte d’Ivoire, Éthiopie, Malawi, Maroc — et deux CER partenaires, la CEEAC et le COMESA. JLMP Action s’est achevée en décembre 2024, JLMP Lead en juin 2025 ; les deux ont fait l’objet d’une évaluation indépendante conjointe portant sur juin 2021 – juin 2025. Le déploiement, lui, était un choix pilote, financé par la SIDA depuis la phase « JLMP Priority » en 2018.\n\nUne troisième phase a été lancée le 30 avril 2026 à Addis-Abeba, dotée de 16 millions de dollars et portée par la Commission de l’Union africaine avec l’OIM, l’OIT et la GIZ.", en: "Targeted rollout in five member states — Cameroon, Côte d’Ivoire, Ethiopia, Malawi, Morocco — and two partner RECs, ECCAS and COMESA. JLMP Action closed in December 2024, JLMP Lead in June 2025; both were covered by a joint independent evaluation running from June 2021 to June 2025. The rollout itself was a pilot approach, funded by SIDA since the \"JLMP Priority\" phase in 2018.\n\nA third phase was launched on 30 April 2026 in Addis Ababa, endowed with USD 16 million and carried by the African Union Commission with IOM, ILO and GIZ." }, lang)}</Prose>
                </div>
              ...
              <div className="mt-8 pt-8 border-t" style={{ borderColor: 'color-mix(in oklab, var(--accent) 26%, var(--paper))' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-white/70 p-4 rounded-lg border border-teal-100">
                    <span className="surtitre surtitre--accent">{tr({ fr: "PALIM — Maroc ↔ Belgique (depuis 2019)", en: "PALIM — Morocco ↔ Belgium (since 2019)" }, lang)}</span>
                    <Prose className="text-xs text-teal-900 leading-relaxed" lang={lang}>{tr({ fr: "120 diplômés formés aux métiers du numérique, dont 40 sont partis travailler en Flandre et 80 sont restés au Maroc. La répartition est le résultat attendu du dispositif et non son échec : la formation financée en commun bénéficie aux deux économies, que le diplômé parte ou reste.", en: "120 graduates trained in digital occupations, of whom 40 went to work in Flanders and 80 stayed in Morocco. That split is the arrangement’s intended outcome rather than its failure: jointly funded training benefits both economies, whether the graduate leaves or stays." }, lang)}</Prose>
                  </div>
                  <div className="bg-white/70 p-4 rounded-lg border border-teal-100">
                    <span className="surtitre surtitre--accent">{tr({ fr: "THAMM / THAMM Plus (2019-2027)", en: "THAMM / THAMM Plus (2019-2027)" }, lang)}</span>
                  ...
                  <div className="bg-white/70 p-4 rounded-lg border border-teal-100">
                    <span className="surtitre surtitre--accent">{tr({ fr: "Sénégal / Ghana ↔ Allemagne (dès 2026)", en: "Senegal / Ghana ↔ Germany (from 2026)" }, lang)}</span>
              ...
              <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="flex items-center text-lg font-serif font-bold text-slate-800 mb-6 border-b border-slate-100 pb-3">
                  ...
                  {tr({ fr: "Produire ses propres diagnostics plutôt que les recevoir", en: "Producing its own diagnoses rather than receiving them" }, lang)}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 stagger">
                  <AuAgencyCard
                    lang={lang}
                  ...
                  >
                    {tr({ fr: "Bureau technique spécialisé de l’UA, institué dans le sillage de la Déclaration de New York (2016) et de l’Objectif 1 du Pacte mondial (données factuelles), lancé le 18 décembre 2020. Il doit doter le continent de données migratoires centralisées, harmonisées et opportunes, condition pour que l’Afrique produise ses propres diagnostics au lieu de les recevoir. Son financement demeure toutefois largement extrabudgétaire, ce qui fait dépendre la continuité de ses travaux de contributions reconduites d’exercice en exercice.", en: "AU specialised technical office, established in the wake of the New York Declaration (2016) and Objective 1 of the Global Compact (evidence base), launched on 18 December 2020. Its remit is to give the continent centralised, harmonised and timely migration data, the condition for Africa to produce its own diagnoses rather than receive them. Its funding nonetheless remains largely extrabudgetary, which makes the continuity of its work dependent on contributions renewed from one financial year to the next." }, lang)}
                  </AuAgencyCard>
                  ...
                  >
                    {tr({ fr: "Le Conseil exécutif ayant accepté l’offre d’accueil du Kenya (décision EX.CL/Dec.808(XXIV)), l’accord de siège est signé et l’Institut lancé le 28 novembre 2014 ; il est hébergé par la Kenya School of Monetary Studies. Son mandat tient en trois objectifs enchaînés : abaisser le coût d’envoi d’argent vers l’Afrique et à l’intérieur du continent, améliorer la mesure et la déclaration de ces transferts dans les États membres, et convertir les flux ainsi mieux connus en effet économique et social. L’AIR relève donc autant du droit bancaire que de la statistique migratoire, ce qui explique qu’il siège auprès d’une école de banque centrale plutôt qu’auprès d’un institut de statistique.", en: "After the Executive Council accepted Kenya’s offer to host it (decision EX.CL/Dec.808(XXIV)), the headquarters agreement was signed and the Institute launched on 28 November 2014; it is hosted by the Kenya School of Monetary Studies. Its mandate rests on three linked objectives: lowering the cost of sending money to and within Africa, improving the measurement and reporting of those transfers in member states, and converting the flows thus better known into economic and social effect. The AIR therefore belongs as much to banking law as to migration statistics, which is why it sits with a central-bank school rather than with a statistical institute." }, lang)}
                  </AuAgencyCard>
        ...
        {activeSdgzTab === 'matrix' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="bg-slate-900 text-white p-6 md:p-8 rounded-xl shadow-md border border-slate-800">
              <span className="surtitre text-blue-400">
            ...
            {(() => {
              ...
              return (
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <Prose className="note-source" lang={lang}>{tr({ fr: "Survolez un effectif pour afficher les pays concernés. Les paliers sans aucun pays ne sont pas représentés.", en: "Hover a count to reveal the countries concerned. Bands with no country are not shown." }, lang)}</Prose>
                </div>
              );
            })()}
            ...
            {matrixView === 'table' && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in">
                  ...
                  <table className="w-full text-start">
                    ...
                    <thead>
                      <tr className="bg-slate-100 border-b border-slate-200 text-slate-600">
                        ...
                        <th scope="col" className="py-4 px-4 font-bold text-[10px] uppercase tracking-widest">{tr({ fr: "Pays", en: "Country" }, lang)}</th>
                        <th scope="col" className="py-4 px-4 font-bold text-[10px] uppercase tracking-widest">{tr({ fr: "Seuil légal visiteur", en: "Legal Visitor Threshold" }, lang)}</th>
                        <th scope="col" className="py-4 px-4 font-bold text-[10px] uppercase tracking-widest">{tr({ fr: "Notes sur l’Obligation de résidence", en: "Notes on the Residence Obligation" }, lang)}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                ...
                <div className="px-5 py-4 border-t border-slate-200 bg-slate-50">
                  ...
                  <Prose className="note-source" lang={lang}>{tr({ fr: "Sources : annonces officielles nationales et Africa Visa Openness Index (BAD/CUA, 2024). Même symbolique que dans l’Explorateur.", en: "Sources: official national announcements and the Africa Visa Openness Index (AfDB/AUC, 2024). Same symbols as in the Explorer." }, lang)}</Prose>
                </div>
              </div>
            )}
...
const LibraryCard = ({ item, lang, essential = false }) => {
  ...
  return (
    ...
    >
      {/* La nature et l’année passent en pied de fiche. La grille égalise la
          hauteur d’une rangée ; le vide tombait donc sous la description des
          fiches courtes — jusqu’à 95 px pendants. Poussées en bas, elles
          alignent leur ligne d’une fiche à l’autre et le vide se range entre
          le texte et elles, ce qui se lit comme une intention. L’ordre du
          document ne bouge pas : on lit toujours la nature avant le titre. */}
      <div className="fiche-nature flex items-center justify-between gap-2">
...
const TabLibrary = ({ text, lang, exportLibraryCSV, children }) => {
  ...
  const matchesSearch = (item) => {
    ...
    return sansAccents(item.title).includes(sansAccents(query)) || sansAccents(item.desc.fr).includes(sansAccents(query)) || sansAccents(item.desc.en).includes(sansAccents(query));
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div>
        <h2 className="surtitre flex items-center">
                  ...
                  {theme.items.map((ind, j) => (
                    ...
                    >
                      ...
                      <div data-ouvert={expandedIndicator === ind.id ? 'true' : 'false'}
                        ...
                        <div className="space-y-4">
                          <div>
                            <span className="surtitre flex items-center gap-1"><Search className="w-3 h-3" />{tr({ fr: "Méthodologie et collecte", en: "Methodology and collection" }, lang)}</span>
    ...
    <div className={compact ? '' : 'bg-white p-7 rounded-lg border border-slate-200 shadow-sm'}>
        ...
        {censusRoundMeta.map((r) => {
          ...
          return (
            <div key={r.key} className={`rounded-md border px-2 py-2 text-center ${tone}`}
              ...
              <span className="surtitre opacity-70">{tr(r.label, lang)}</span>
      ...
      {!compact && (
        <Sources
          lang={lang}
          items={[
            L("Division de statistique des Nations unies (UNSD)", "United Nations Statistics Division (UNSD)"),
            L("UN DESA", "UN DESA"),
          ]}
          note={L("Compilation de l’auteur (Ben Mokhtar, 2024) ; statuts du cycle 2020 vérifiés en août 2026 auprès des instituts nationaux.",
                  "Author’s own compilation (Ben Mokhtar, 2024); 2020-round statuses verified in August 2026 against national statistical institutes.")}
        />
      )}
    </div>
  );
  ...
  return (
    <section className="bg-white border border-slate-200 overflow-hidden" aria-labelledby={idTitre}>
      ...
      >
        ...
        <span className="pliure-mot shrink-0 mt-0.5">
          {ouvert ? L('Replier', 'Collapse', { ar: 'طيّ' })
                  : L('Lire', 'Read', { ar: 'اقرأ' })}
          <ChevronDown className={`w-4 h-4 shrink-0 transition-transform ${ouvert ? 'rotate-180' : ''}`}
                       aria-hidden="true" />
        </span>
      </button>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Reperes
        ...
        chapeau={{
          fr: "Aucune institution ne dénombre directement les migrants du monde : les chiffres qui circulent sont des agrégats, construits à partir de ce que chaque État a lui-même mesuré, avec ses instruments et selon son calendrier. Or ces instruments ne sont pas équivalents. Un recensement, un registre de séjour, une enquête auprès des ménages et un poste-frontière ne saisissent ni la même population, ni la même période, ni le même événement. Identifier celui dont un chiffre provient est donc l’opération à conduire avant toute interprétation, et elle commande la lecture de ce qui suit.",
          en: 'No institution counts the world’s migrants directly: the figures in circulation are aggregates, built from what each state has measured for itself, with its own instruments and on its own schedule. Those instruments are not equivalent. A census, a residence register, a household survey and a border post capture neither the same population, nor the same period, nor the same event. Identifying which one a figure comes from is therefore the operation to carry out before any interpretation, and it governs how everything below reads.'
        }}
        notions={[
          { mot: { fr: 'Stock', en: 'Stock' },
            sens: { fr: "Le nombre de personnes nées à l’étranger qui résident dans un pays à une date donnée. La mesure est instantanée : elle décrit un état de la population, et non les mouvements qui l’ont produit. La quasi-totalité des chiffres cités dans le débat public sont des stocks.",
                    en: 'The number of people born abroad who reside in a country on a given date. The measurement is instantaneous: it describes a state of the population, not the movements that produced it. Almost every figure quoted in public debate is a stock.' } },
          { mot: { fr: 'Flux', en: 'Flow' },
            sens: { fr: "Le nombre de personnes ayant franchi une frontière au cours d’une période. La mesure porte sur des événements et non sur un état, ce qui suppose un enregistrement continu que peu d’appareils statistiques tiennent. C’est pourtant un flux que le mot « arrivées » désigne, alors que le chiffre qui l’accompagne est presque toujours un stock.",
                    en: 'The number of people who crossed a border over a period. The measurement bears on events rather than on a state, which presupposes continuous recording that few statistical systems maintain. Yet a flow is what the word “arrivals” denotes, while the figure attached to it is almost always a stock.' } },
          { mot: { fr: 'Millésime', en: 'Vintage' },
            sens: { fr: "L’année à laquelle la mesure se rapporte, qui n’est pas celle de sa publication : un rapport paru en 2024 peut porter des stocks établis en 2020. Un chiffre cité sans son millésime ne peut donc être ni comparé à un autre, ni rapporté à un événement daté.",
                    en: 'The year the measurement refers to, which is not the year of publication: a report issued in 2024 may carry stocks established in 2020. A figure quoted without its vintage can therefore be neither compared with another nor related to a dated event.' } },
          { mot: { fr: 'Compilateur', en: 'Compiler' },
            sens: { fr: "L’organisation qui rassemble et harmonise les mesures nationales — UN DESA, HCR, IDMC, OIT, Banque mondiale. Elle ne produit pas la mesure : elle réunit ce que les États déclarent, dans l’état où ils le déclarent. Les lacunes des appareils nationaux se retrouvent donc, telles quelles, dans l’agrégat mondial.",
                    en: 'The body that gathers and harmonises national measurements — UN DESA, UNHCR, IDMC, ILO, the World Bank. It does not produce the measurement: it assembles what states declare, in the state in which they declare it. The gaps in national systems therefore carry over, unchanged, into the global aggregate.' } },
        ]}
        colonnes={[
          { fr: 'Source', en: 'Source' },
        ...
        lignes={[
          { source: { fr: 'Recensement', en: 'Census' },
            mesure: { fr: "Tous les résidents, pays de naissance compris, à un intervalle d’environ dix ans.", en: 'Every resident, country of birth included, at intervals of about ten years.' },
            angle: { fr: "Ceux qui sont partis, et tout ce qui se produit entre deux passages.", en: 'Those who have left, and everything that happens between two rounds.' } },
          { source: { fr: 'Registres administratifs', en: 'Administrative registers' },
            ...
            angle: { fr: "Les personnes sans titre de séjour, qui résident sans figurer nulle part.", en: 'People without a residence permit, who live there without appearing anywhere.' } },
          { source: { fr: 'Enquêtes auprès des ménages', en: 'Household surveys' },
            mesure: { fr: "Les motifs du départ, les conditions de vie, les sommes envoyées au pays.", en: 'Reasons for leaving, living conditions, sums sent home.' },
            angle: { fr: "Le détail par pays d’origine : l’échantillon est trop mince pour y descendre.", en: 'Detail by country of origin: the sample is too thin to reach that level.' } },
        ...
        pied={{
          fr: "Trois conséquences pratiques en découlent. Un chiffre privé de son millésime ne peut être interprété, puisque rien n’indique à quel état de la population il se rapporte. Un pourcentage privé de son dénominateur ne le peut pas davantage : la même proportion ne dit pas la même chose selon qu’elle porte sur les 47 États ayant recensé ou sur les 54 que compte l’Union africaine. Enfin, deux institutions peuvent publier de bonne foi deux valeurs différentes du même phénomène dès lors qu’elles ne dénombrent pas la même population : c’est le cas des personnes déplacées internes, suivies selon des périmètres distincts par l’IDMC et par le HCR. Cette plateforme affiche alors les deux valeurs plutôt que d’en retenir une. Cadre de référence : Nations unies, Recommandations en matière de statistiques des migrations internationales, révision 1.",
          en: 'Three practical consequences follow. A figure stripped of its vintage cannot be interpreted, since nothing indicates which state of the population it refers to. A percentage stripped of its denominator cannot be either: the same proportion does not say the same thing depending on whether it bears on the 47 states that held a census or on the 54 that make up the African Union. Finally, two institutions may in good faith publish two different values for the same phenomenon whenever they are not counting the same population: this is the case for internally displaced persons, monitored under distinct perimeters by IDMC and by UNHCR. This platform then shows both values rather than settling on one. Reference framework: United Nations, Recommendations on Statistics of International Migration, Revision 1.'
        }}
      />
      ...
      <MovementOpener
        ...
        thesis={L(
          "La collecte primaire n’est pas le point faible de la statistique migratoire africaine : quarante-sept États sur cinquante-quatre ont recensé sur le cycle 2010, à un intervalle moyen de 11,1 ans pour une norme onusienne de dix ans.",
          "Primary collection is not the weak point of African migration statistics: forty-seven states out of fifty-four held a census over the 2010 round, at an average interval of 11.1 years against a UN standard of ten."
        )}
      />
      ...
      {/* L'Afrique comparee. Le recit d'une Afrique « sans donnees » ne se
          refute pas par une affirmation contraire : il se verifie sur la seule
          mesure qui vaille ici — la part des pays d'une region disposant d'une
      ...
      <Reveal delay={40} className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
        ...
        <Prose className="text-sm text-slate-600 mb-5" lang={lang}>{L(
          "Le déficit de données que l’on prête au continent peut être mesuré, et il l’est. UN DESA publie, pour chacune de ses régions, la part des pays disposant d’au moins une source empirique depuis 2010 sur trois points : le nombre total de migrants, leur répartition par sexe et leur pays d’origine. Le troisième commande les deux autres, puisque sans pays d’origine aucun corridor ne peut être établi et que la mobilité se réduit alors à un volume sans direction.",
          "The data deficit ascribed to the continent can be measured, and it has been. UN DESA publishes, for each of its regions, the share of countries holding at least one empirical source since 2010 on three points: the total number of migrants, their distribution by sex, and their country of origin. The third governs the other two, since without country of origin no corridor can be established and mobility is reduced to a volume with no direction."
        )}</Prose>

        <ReservePerimetre lang={lang} />

        <CompareRegions
          lignes={COUVERTURE_STATISTIQUE} champ="origine" lang={lang}
          repere={COUVERTURE_STATISTIQUE.find(r => r.region === 'monde').origine}
          titre={L("Pays disposant d’une source sur le pays d’origine des migrants, depuis 2010",
                   "Countries with a data source on migrants’ country of origin, since 2010")}
        />

        <Prose className="text-sm text-slate-600 mt-5" lang={lang}>{L(
          "Le classement se déplace sur les deux autres mesures : 84 % des pays d’Afrique subsaharienne documentent le nombre total de migrants, contre 87 % dans le monde, et 65 % leur répartition par sexe, contre 71 %. L’écart est donc réel et de l’ordre de trois à six points, quand le déficit statistique africain est couramment présenté comme une absence de données — ce que ces trois mesures contredisent.",
          "The ranking shifts on the other two measures: 84 per cent of Sub-Saharan African countries document the total number of migrants, against 87 per cent worldwide, and 65 per cent their distribution by sex, against 71. The gap is therefore real and of the order of three to six points, whereas the African statistical deficit is commonly presented as an absence of data — which these three measures contradict."
        )}</Prose>

        <Sources lang={lang} items={[{ label: tr(SOURCE_MONDE.label, lang), url: SOURCE_MONDE.url }]}
                 note={L("Tableau 1 de la publication. La couverture décrit l’existence d’une source, pas sa qualité ni sa périodicité.",
                         "Table 1 of the publication. Coverage describes the existence of a source, not its quality or its periodicity.")} />
      </Reveal>
        ...
        >
          ...
          <div className="aparte">
            ...
            <Prose className="text-xs text-slate-500 mt-3 leading-relaxed" lang={lang}>{L(
                "C’est l’étalon pertinent pour situer la performance africaine : l’intervalle moyen observé sur le continent est de 11,1 ans, soit un écart d’un an à la recommandation décennale — un ordre de grandeur sans rapport avec le retard structurel que l’on prête d’ordinaire aux appareils statistiques africains.",
                "This is the relevant yardstick for situating African performance: the average interval observed across the continent is 11.1 years, an overshoot of one year on the decennial recommendation — an order of magnitude unrelated to the structural backwardness usually ascribed to African statistical systems."
              )}</Prose>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1">
              {(tr({ fr: [
                ...
                <li key={i} className="flex items-baseline justify-between gap-3 py-1.5 text-[13px]" style={{ borderBottom: '1px solid var(--rule)' }}>
                  ...
                  <span className="surtitre shrink-0"
            ...
            <Prose className="text-xs text-slate-500 mt-3 leading-relaxed text-justify" lang={lang}>{L(
                "Trois de ces quatre caractéristiques sont des « thèmes centraux », que la norme recommande de collecter dans tout recensement. L’instrument de mesure des migrations est donc déjà inscrit dans le standard que les États africains appliquent, et ce qui manque ensuite ne peut être imputé à la phase de collecte. La perte se produit en aval, au dépouillement et à la publication.",
                "Three of these four characteristics are \"core topics\", which the standard recommends every census collect. The instrument for measuring migration is therefore already written into the standard African states apply, and what goes missing afterwards cannot be charged to the collection stage. The loss occurs downstream, in tabulation and publication."
              )}</Prose>
          </div>
        </AfricanCounterpoint>
      </Reveal>
      ...
      <MovementOpener
        ...
        thesis={L(
          "Reste à savoir où, exactement : dans ce qui remonte des instituts nationaux, dans ce qui est harmonisé entre eux, ou dans la profondeur des questions posées.",
          "It remains to establish where exactly: in what national institutes report, in what is harmonised between them, or in the depth of the questions asked."
        )}
      />
      ...
      <Reveal delay={40} className="bg-white rounded-xl p-8 md:p-10 border border-slate-200 shadow-sm">
        ...
        <div className="mb-8">
          ...
          <p className="text-[11px] mt-3 leading-relaxed" style={{ color: 'var(--label)' }}>
            ...
            {L("Le cycle 2030 vient de s’ouvrir (fenêtre 2025-2034) : sa barre ne compte encore que les opérations abouties en moins de deux ans, et elle continuera de monter jusqu’en 2034.",
               "The 2030 round has just opened (2025-2034 window): its bar so far counts only the operations completed in under two years, and it will keep rising until 2034.")}
          </p>
        ...
        <div className="aparte mt-5">
          <p className="surtitre">
            {L("Ce que le croisement avec la norme révèle", "What the overlay with the standard reveals")}
          </p>
          <Prose className="text-[13px] text-slate-600 leading-relaxed text-justify" lang={lang}>{L(
              "La norme onusienne place trois caractéristiques sur le même plan, toutes « thèmes centraux » : pays de naissance, citoyenneté, année d’arrivée. Les deux premières sont presque universelles dans les recensements africains — 87 % et 91,5 %. La troisième tombe à 15 %.\n\nCe n’est ni une question de moyens ni une question de capacité : les trois figurent dans le même questionnaire de référence, et deux sont posées. Ce qui décroche est précisément la question qui permettrait de dater les trajectoires, donc de les analyser. Le motif du départ, lui, est posé par six États — et il ne figure même pas parmi les caractéristiques prévues par la norme (Ben Mokhtar, 2026).",
              "The UN standard places three characteristics on the same footing, all \"core topics\": country of birth, citizenship, year of arrival. The first two are near-universal in African censuses — 87 and 91.5 per cent. The third falls to 15.\n\nThis is neither a question of means nor of capacity: all three appear in the same model questionnaire, and two are asked. What drops away is precisely the question that would allow trajectories to be dated, and therefore analysed. The reason for leaving is asked by six states — and it does not even figure among the characteristics the standard provides for (Ben Mokhtar, 2026)."
            )}</Prose>
        </div>
        <Prose className="note-source" lang={lang}>{L(
      ...
      <MovementOpener
        ...
        thesis={L(
          "À ce diagnostic, l’Union africaine a répondu par une stratégie continentale d’harmonisation statistique, dont cette plateforme prolonge l’ambition avec ses propres instruments.",
          "To this diagnosis the African Union responded with a continental statistical harmonisation strategy, whose ambition this platform extends with instruments of its own."
        )}
      />
      ...
      <Reveal delay={40} className="bg-slate-900 rounded-xl p-8 md:p-10 border border-slate-800 shadow-sm text-white">
        ...
        <div className="space-y-4 text-sm text-slate-300 leading-relaxed max-w-4xl text-justify">
          <Prose lang={lang}>{L(
            "Le goulot d’étranglement se situe après la collecte. Une part importante du traitement des données de recensement africaines a longtemps été assurée depuis l’extérieur, le Bureau du recensement des États-Unis (USCB) ayant accompagné pendant des décennies le dépouillement de ces données. Cette dépendance ne porte pas sur la production de la donnée brute, qui existe et que les instituts nationaux collectent eux-mêmes, mais sur ce qui vient après : la mise en forme, l’interprétation, et le choix des catégories dans lesquelles la population est rangée.",
...
const TabGlossary = ({ lang, text, exportGlossaryCSV, children }) => {
  ...
  const filteredCategories = glossaryData
    .map(cat => ({
      ...cat,
      terms: cat.terms
        .filter(t => !query || sansAccents(t.term).includes(sansAccents(query)) || sansAccents(t.en_term).includes(sansAccents(query)) || sansAccents(t.fr).includes(sansAccents(query)) || sansAccents(t.en).includes(sansAccents(query)))
                ...
                <div key={tIdx} className="p-4 border border-slate-200 bg-slate-50">
                  ...
                  {t.source && (
                    <div className="mt-3 pt-2.5" style={{ borderTop: '1px solid var(--rule)' }}>
                      <span className="surtitre me-1.5">
...
const TabMethodology = ({ text, lang, children }) => (
  <div className="space-y-8 animate-in fade-in duration-500">
    ...
    <Reveal className="bg-white rounded-xl p-8 md:p-10 border border-slate-200 shadow-sm">
      ...
      <Prose className="text-sm text-slate-500 leading-relaxed max-w-3xl mb-8" lang={lang}>{tr({ fr: "La plateforme ne produit aucune donnée primaire : elle consolide des séries publiques et documente le traitement qu’elle leur applique. Les cinq étapes qui suivent sont appliquées à chaque indicateur, dans cet ordre, et chacune est vérifiable sur la fiche de l’indicateur concerné.", en: "The platform produces no primary data: it consolidates public series and documents the processing it applies to them. The five stages below are applied to every indicator, in this order, and each can be checked on the relevant indicator’s card." }, lang)}</Prose>
        ...
        {methodConventions.map((c, i) => (
          <div key={i} className="border-s-2 border-teal-200 ps-4">
            <dt className="flex flex-wrap items-baseline gap-2 mb-1.5">
              <span className="surtitre">{tr(c.label, lang)}</span>
    ...
    <section className="bg-white rounded-xl p-8 md:p-10 border shadow-sm relative border-slate-200">
      ...
      <div className="mt-8 pt-6 border-t border-slate-100">
        <p className="surtitre mb-4">{text.method.sources_title}</p>
      ...
      <div className="mt-8 pt-6 border-t border-slate-100">
        ...
        <div className="bg-slate-50 p-5 rounded-lg border border-slate-200">
          <Prose className="text-xs text-slate-700 leading-relaxed" lang={lang}>{tr({ fr: "Les sous-régions affichées dans l’Explorateur et dans la matrice « Entrées & séjours » suivent le découpage officiel de l’Union africaine, en cinq régions. UN DESA publie les siens selon la nomenclature M49 des Nations unies : les deux grilles divergent sur sept pays.\n\nLa Mauritanie est en Afrique du Nord pour l’UA, en Afrique de l’Ouest pour l’ONU. Le Burundi et le Rwanda passent d’Afrique centrale à Afrique de l’Est. Le Malawi, le Mozambique, la Zambie et le Zimbabwe, d’Afrique australe à Afrique de l’Est.\n\nLes sous-totaux par région affichés ici ne coïncideront donc pas exactement avec les tableaux régionaux publiés directement par UNDESA pour ces pays. Ce choix aligne le site sur le cadrage institutionnel de l’Union africaine utilisé par ailleurs dans la section Gouvernance.", en: "The sub-regions shown in the Explorer and in the \"Entry & Residence\" matrix follow the African Union’s official five-region breakdown — not the UN M49 classification UNDESA uses to publish its own migrant stock tables. The two groupings diverge on seven countries.\n\nMauritania sits in North Africa under the AU, in West Africa under the UN. Burundi and Rwanda move from Central to East Africa. Malawi, Mozambique, Zambia and Zimbabwe, from Southern to East Africa.\n\nAs a result, the regional subtotals shown here will not exactly match UNDESA’s own published regional tables for these countries. This choice aligns the site with the African Union institutional framing used elsewhere in the Governance section." }, lang)}</Prose>
        </div>
      </div>
    </section>
  </div>
    ...
    <section className="space-y-8 animate-in fade-in duration-500">
      ...
      <Reperes
        ...
        notions={[
          { mot: { fr: 'Indépendante', en: 'Independent' },
            sens: { fr: "Le projet ne répond à aucune commande et ne reçoit aucun financement institutionnel, de sorte que le périmètre retenu, les définitions employées et les sources écartées n’engagent que leur auteur.", en: 'The project answers to no commission and receives no institutional funding, so that the perimeter adopted, the definitions used and the sources set aside commit no one but their author.' } },
          { mot: { fr: 'Adossée, pas confondue', en: 'Backed by, not merged with' },
            sens: { fr: "Le cadre conceptuel provient d’une thèse en cours, mais la plateforme n’en est ni le résumé ni la publication : elle n’expose que des données déjà publiques et les notions qui servent à les lire.", en: 'The conceptual frame comes from an ongoing thesis, but the platform is neither its summary nor its publication: it presents only data that is already public, and the concepts used to read it.' } },
          { mot: { fr: 'Ce qu’elle n’est pas', en: 'What it is not' },
            sens: { fr: "Elle n’est ni un observatoire officiel, ni une source primaire, ni un service de veille en temps réel : elle ne mesure rien elle-même et ne se prévaut d’aucun mandat institutionnel.", en: 'It is neither an official observatory, nor a primary source, nor a real-time monitoring service: it measures nothing itself and claims no institutional mandate.' } },
          { mot: { fr: 'En développement', en: 'In development' },
            sens: { fr: "Les contenus s’enrichissent au fil des vérifications, et une donnée manquante y est signalée comme telle plutôt que remplacée par une estimation.", en: 'Content grows as verification proceeds, and a missing figure is flagged as missing rather than replaced by an estimate.' } },
        ]}
      />
      {children}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-4 text-sm text-slate-600 leading-relaxed text-justify">
            ...
            <p className="surtitre text-emerald-700 mt-5">
          ...
          <div className="mt-9 pt-8 border-t border-slate-200">
            <h3 className="surtitre flex items-center text-slate-500 mb-4">
      ...
      <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex flex-wrap items-center gap-2 px-5 py-3">
          <span className="surtitre" style={{ letterSpacing: '.16em', color: 'var(--label)' }}>
        ...
        {voletAuteur === 'publications' && (
          <div className="border-t border-slate-100 animate-in fade-in duration-300">
            ...
            <span className="surtitre">{tr(pub.kind, lang)}</span>
            {pub.url && (
        ...
        {voletAuteur === 'medias' && (
          <div className="border-t border-slate-100 px-7 py-5 animate-in fade-in duration-300">
            ...
            {tr(m.title, lang)}
            {/youtu\.?be/.test(m.url)
              ? <span className="surtitre not-italic inline-flex items-center gap-1 shrink-0 mt-0.5" style={{ color: 'var(--accent-deep)' }}>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm group hover:border-blue-300 transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
          <div className="flex items-center space-x-2 rtl:space-x-reverse">
            ...
            <h4 className="surtitre">{text.about.citation_title}</h4>
          </div>
...
export default function App() {
  ...
  // Chaque analyse devient citable : un identifiant tiré de son titre, et un
  ...
  // Le volet d'ouverture est la signature, pas la bibliothèque : on arrive par
  // « qui fait cela », on descend vers l'appareil.
  const [activeResourceTab, setActiveResourceTab] = useState('about');
  ...
  const filteredCountries = useMemo(() => {
    return currentCountries.filter(c => {
      ...
      return sansAccents(cName).includes(sansAccents(searchTerm));
  ...
  const display = useMemo(() => {
    ...
    return {
      ...
      // Vérifiée pour l’Union entière, incalculable pour une sous-région — et
      // surtout pas « 50 » par défaut, qui n’est pas une valeur mais un nombre
      // inventé ayant l’air d’une mesure.
      retention: activeSubRegion === 'all' ? INTRA_AFRICAIN_UA.part : null,
  ...
  useEffect(() => {
    ...
    const NOMS = {
      atlas: { fr: 'Atlas', en: 'Atlas' },
      home: { fr: 'Accueil', en: 'Home' },
      evidence: { fr: 'Vérification des affirmations', en: 'Evidence Check' },
  ...
  const navigation = [
    ...
    { id: 'evidence', icon: Globe, label: { fr: 'Vérification', en: 'Evidence Check', ar: 'فحص الأدلة' } },
    { id: 'data', icon: BarChart3, label: { fr: 'Données', en: 'Data', ar: 'البيانات والإحصاءات' } },
    ...
    // « À propos » porte désormais tout l'appareil du site : la signature, les
    // sources, les mots et la méthode. Quatre volets, une seule entrée.
    { id: 'about', icon: Info, label: { fr: 'À propos & méthode', en: 'About & method', ar: 'عن المنصة والمنهجية' } },
  ];
  ...
  return (
    <div className={`min-h-screen bg-[#f8f9fa] font-sans text-slate-800 text-sm transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'} print:bg-white print:text-black`}>
      ...
      <style type="text/css" media="print">
        {`
          ...
          /* La cle est passee en <th scope="row"> pour qu'un lecteur d'ecran sache
             de quoi la valeur est la valeur. Un <th> etant gras et centre par
      ...
      <nav data-section={activeTab}
        ...
        <div className="border-b border-slate-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between h-[76px] items-center">
            ...
            <button
              type="button"
              onClick={() => allerVers('home')}
              aria-label={tr({ fr: 'Revenir à l’accueil', en: 'Back to home', ar: 'العودة إلى الرئيسية' }, lang)}
              className="marque-accueil flex items-center space-x-3 rtl:space-x-reverse min-w-0"
            >
              ...
              <BrandLockup corps={19} tone="dark" nature={false} ecart={0.42} className="shrink-0" />
            </button>
            <div className="barre-outils flex items-center gap-2 shrink-0">
      ...
      <main id="contenu" tabIndex={-1} data-section={activeTab} className={`${mainMaxWidth} mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-6 ${showModal ? 'print:hidden' : ''}`}>
        {activeTab === 'atlas' && (
          <TabAtlas
          ...
          <div className="space-y-8">
            {(() => {
              const resourceSwitch = (
                <div className="bascule-groupe flex max-w-2xl">
                  {/* La signature ouvre le groupe : on arrive par « qui fait
                      cela », on descend vers l’appareil. */}
                  <button
                    onClick={() => setActiveResourceTab('about')}
                    data-actif={activeResourceTab === 'about' ? 'true' : 'false'}
                    className="bascule flex-1 justify-center transition-all"
                  >
                    <Info className="w-3.5 h-3.5 hidden sm:block" aria-hidden="true" /> {tr({ fr: 'À propos', en: 'About', ar: 'عن المنصة' }, lang)}
                  </button>
                  <button
              ...
              if (activeResourceTab === 'about')
                return <TabAbout text={text} lang={lang}>{resourceSwitch}</TabAbout>;
      ...
      {/* Le vide avant le pied faisait deux cents pixels — quatre-vingts de
          marge plus les marges du dernier bloc. Il prend le pas d'écart entre
          cartes, qui est celui de tout le reste de la page. */}
      <footer className="print:hidden" style={{ backgroundColor: 'var(--plan-encre)', marginTop: 'var(--pas-3)' }}>
        {/* Le filet de signature portait un indigo écrit en dur : le même dans
            les neuf sections, quelle que soit leur teinte. Il prend l'accent. */}
        <div className="h-px w-full" style={{ background: 'linear-gradient(90deg, var(--accent), color-mix(in oklab, var(--accent) 38%, transparent) 40%, transparent)' }} />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8">

            {/* Marque : le pied a la hauteur qu'il faut au logotype complet,
                ce que la barre de navigation n'a pas. */}
            <div className="md:col-span-5">
              <BrandLockup corps={26} tone="dark" className="mb-5" />
              <Prose className="text-sm leading-relaxed max-w-sm" style={{ color: 'var(--reserve-douce)' }} lang={lang}>{text.footer.tag}</Prose>
            </div>

            {/* CITER PLUTÔT QUE NAVIGUER. Une seconde liste des sept onglets, à
                trois cents pixels du menu resté collé en haut, n'apprenait rien.
                La référence, elle, ne se trouve nulle part ailleurs au moment où
                on en a besoin — c'est-à-dire quand on vient de finir de lire. */}
            <div className="md:col-span-4">
              <span className="surtitre surtitre--clair">
                {tr({ fr: 'Citer cette plateforme', en: 'Cite this platform' }, lang)}
              </span>
              <Prose className="text-xs leading-relaxed" style={{ color: 'var(--reserve-douce)' }} lang={lang}>
          ...
          <div className="mt-10 pt-6 flex flex-col sm:flex-row justify-between gap-3" style={{ borderTop: '1px solid color-mix(in oklab, var(--reserve) 10%, transparent)' }}>
            <p className="text-[13px]" style={{ color: 'var(--reserve-tenue)' }}>
              © 2026 Yassine Ben Mokhtar — {tr({ fr: 'recherche indépendante', en: 'independent research' }, lang)}
            </p>
            <Prose className="text-[13px]" style={{ color: 'var(--reserve-tenue)' }} lang={lang}>{tr({ fr: 'Données publiques consolidées, chaque chiffre avec sa source et son millésime.', en: 'Consolidated public data; every figure with its source and vintage.' }, lang)}</Prose>
          </div>
        </div>
      </footer>
    </div>
